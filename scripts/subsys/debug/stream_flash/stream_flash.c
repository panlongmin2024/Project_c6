...
#define SYS_LOG_MODULE_NAME STREAM_FLASH
#define SYS_LOG_LEVEL CONFIG_STREAM_FLASH_SYS_LOG_LEVEL
				  ...
				  void *param)
{
	...
	if (settings_name_next(key, NULL) == 0) {
		...
		if (len != sizeof(ctx->bytes_written)) {
			SYS_LOG_ERR("Unable to read bytes_written from storage");
			return len;
		}
		...
		if (bytes_written >= ctx->bytes_written) {
			...
			SYS_LOG_WRN("Loaded outdated bytes_written %zu < %zu",
		...
		if (ctx->bytes_written > 0) {
			rc = flash_get_page_info_by_offs(ctx->fdev, offset,
							 &page);
			if (rc != 0) {
				SYS_LOG_ERR("Error %d while getting page info", rc);
				return rc;
			}
...
int stream_flash_erase_page(struct stream_flash_ctx *ctx, off_t off)
{
	...
	SYS_LOG_DBG("Erasing page at offset 0x%08lx", (long)page.start_offset);

	rc = flash_erase(ctx->fdev, page.start_offset, page.size);

	if (rc != 0) {
		SYS_LOG_ERR("Error %d while erasing page", rc);
	} else {
		ctx->last_erased_page_start_offset = page.start_offset;
	}
...
static int flash_sync(struct stream_flash_ctx *ctx)
{
	...
	if (IS_ENABLED(CONFIG_STREAM_FLASH_ERASE)) {
		...
		if (rc < 0) {
			SYS_LOG_ERR("stream_flash_erase_page err %d offset=0x%08zx",
				rc, write_addr);
			return rc;
		}
	}
	...
	if (rc != 0) {
		SYS_LOG_ERR("flash_write error %d offset=0x%08zx", rc,
			write_addr);
		return rc;
	}
	...
	if (ctx->callback) {
		...
		rc = flash_read(ctx->fdev, write_addr, ctx->buf,
				ctx->buf_bytes);
		if (rc != 0) {
			SYS_LOG_ERR("flash read failed: %d", rc);
			return rc;
		}

		rc = ctx->callback(ctx->buf, ctx->buf_bytes, write_addr);
		if (rc != 0) {
			SYS_LOG_ERR("callback failed: %d", rc);
			return rc;
		}
	}
		      ...
		      stream_flash_callback_t cb)
{
	...
	if (rc != 0) {
		SYS_LOG_ERR("Error %d initializing settings subsystem", rc);
		return rc;
	}
#endif

	if (buf_len % 0x1000) {
		SYS_LOG_ERR("Buffer size is not aligned to minimal write-block-size");
		return -EFAULT;
	}
			       ...
			       const char *settings_key)
{
	...
	if (rc != 0) {
		SYS_LOG_ERR("Error %d while loading progress for \"%s\"",
			rc, settings_key);
	}
			       ...
			       const char *settings_key)
{
	...
	if (rc != 0) {
		SYS_LOG_ERR("Error %d while storing progress for \"%s\"",
			rc, settings_key);
	}
				...
				const char *settings_key)
{
	...
	if (rc != 0) {
		SYS_LOG_ERR("Error %d while deleting progress for \"%s\"",
			rc, settings_key);
	}
