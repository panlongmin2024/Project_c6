...
#define ACT_LOG_MODEL_ID ALF_MODEL_IPSO_LIGHT_CONTROL
#include <stdint.h>
#include <init.h>
#include <net/lwm2m.h>
				 ...
				 bool last_block, size_t total_size)
{
	...
	if (data_len != 4) {
		ACT_LOG_ID_ERR(ALF_STR_on_time_post_write_cb__UNKNOWN_SIZE_U, 1, data_len);
		return 0;
	}
...
static struct lwm2m_engine_obj_inst *light_control_create(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_light_control_create__CREATE_IPSO_LIGHT_CO, 1, obj_inst_id);
