...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_OBJ_SERVER
#include <stdint.h>
#include <init.h>
#include <net/lwm2m.h>
...
static int disable_cb(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_disable_cb__DISABLE_D, 1, obj_inst_id);
...
static struct lwm2m_engine_obj_inst *server_create(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_server_create__CREATE_LWM2M_SERVER_, 1, obj_inst_id);
	return &inst[index];
}
...
static int lwm2m_server_init(struct device *dev)
{
	...
	if (ret < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_server_init__CREATE_LWM2M_SERVER_, 1, ret);
	}
