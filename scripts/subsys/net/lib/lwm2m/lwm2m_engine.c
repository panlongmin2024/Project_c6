...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_ENGINE
...
char *lwm2m_sprint_ip_addr(const struct sockaddr *addr)
{
	...
	ACT_LOG_ID_ERR(ALF_STR_lwm2m_sprint_ip_addr__UNKNOWN_IP_ADDRESS_F, 1, addr->sa_family);
	return NULL;
}
			       ...
			       u16_t format)
{
	...
	if (!addr) {
		ACT_LOG_ID_ERR(ALF_STR_engine_add_observer__SOCKADDR_IS_REQUIRED, 0);
		return -EINVAL;
	}
			  ...
			  struct lwm2m_engine_obj_inst **obj_inst)
{
	...
	*obj_inst = NULL;
	...
	if (!obj) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_create_obj_inst__UNABLE_TO_FIND_OBJ_U, 1, obj_id);
		return -ENOENT;
	}

	if (!obj->create_cb) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_create_obj_inst__OBJ_U_HAS_NO_CREATE_, 1, obj_id);
		return -EINVAL;
	}

	if (obj->instance_count + 1 > obj->max_instance_count) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_create_obj_inst__NO_MORE_INSTANCES_AV, 1, obj_id);
		return -ENOMEM;
	}
		       ...
		       const u8_t *token, u8_t tkl)
{
	...
	*pkt = net_pkt_get_tx(net_ctx, BUF_ALLOC_TIMEOUT);
	if (!*pkt) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_init_message__UNABLE_TO_GET_TX_PAC, 0);
		return -ENOMEM;
	}

	frag = net_pkt_get_data(net_ctx, BUF_ALLOC_TIMEOUT);
	if (!frag) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_init_message__UNABLE_TO_GET_DATA_B, 0);
		net_pkt_unref(*pkt);
		*pkt = NULL;
		return -ENOMEM;
	}
	...
	if (r < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_init_message__ZOAP_PACKET_INIT_ERR, 1, r);
		return r;
	}
...
int lwm2m_engine_create_obj_inst(char *pathstr)
{
	...
	if (path.level != 2) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_create_obj_inst__PATH_MUST_HAVE_2_PAR, 0);
		return -EINVAL;
	}
...
static int lwm2m_engine_set(char *pathstr, void *value, u16_t len)
{
	...
	if (path.level < 3) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_set__PATH_MUST_HAVE_3_PAR, 0);
		return -EINVAL;
	}
	...
	if (!obj_inst->resources || obj_inst->resource_count == 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_set__OBJ_INSTANCE_HAS_NO_, 0);
		return -EINVAL;
	}

	obj_field = lwm2m_get_engine_obj_field(obj_inst->obj, path.res_id);
	if (!obj_field) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_set__OBJ_FIELD_D_NOT_FOUN, 1, path.res_id);
		return -ENOENT;
	}
	...
	if (!res) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_set__RES_INSTANCE_D_NOT_F, 1, path.res_id);
		return -ENOENT;
	}
	...
	if (!data_ptr) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_set__RES_DATA_POINTER_IS_, 0);
		return -EINVAL;
	}
...
static int lwm2m_engine_get(char *pathstr, void *buf, u16_t buflen)
{
	...
	if (path.level < 3) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_get__PATH_MUST_HAVE_3_PAR, 0);
		return -EINVAL;
	}
	...
	if (!obj_inst->resources || obj_inst->resource_count == 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_get__OBJ_INSTANCE_HAS_NO_, 0);
		return -EINVAL;
	}

	obj_field = lwm2m_get_engine_obj_field(obj_inst->obj, path.res_id);
	if (!obj_field) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_get__OBJ_FIELD_D_NOT_FOUN, 1, path.res_id);
		return -ENOENT;
	}
	...
	if (!res) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_engine_get__RES_INSTANCE_D_NOT_F, 1, path.res_id);
		return -ENOENT;
	}
			       ...
			       struct lwm2m_engine_res_inst **res)
{
	...
	if (!obj_inst->resources || obj_inst->resource_count == 0) {
		ACT_LOG_ID_ERR(ALF_STR_engine_get_resource__OBJ_INSTANCE_HAS_NO_, 0);
		return -EINVAL;
	}
	...
	if (!*res) {
		ACT_LOG_ID_ERR(ALF_STR_engine_get_resource__RES_INSTANCE_D_NOT_F, 1, path->res_id);
		return -ENOENT;
	}
			       ...
			       struct lwm2m_engine_res_inst **res)
{
	...
	if (path.level < 3) {
		ACT_LOG_ID_ERR(ALF_STR_engine_get_resource_from_pathstr__PATH_MUST_HAVE_3_PAR, 0);
		return -EINVAL;
	}
	...
	while (obj_inst) {
		...
		for (index = 0; index < obj_inst->resource_count; index++) {
			...
			if (!obj_field) {
				...
				if (ret < 0) {
					/* What to do here? */
					ACT_LOG_ID_ERR(ALF_STR_do_read_op__READ_OP_FAILED_D, 1, ret);
				} else {
					num_read += 1;
					pos = out->outlen;
				}
		       ...
		       u16_t format)
{
	...
	default:
		ACT_LOG_ID_ERR(ALF_STR_do_write_op__UNSUPPORTED_FORMAT_U, 1, format);
		return -EINVAL;
			  ...
			  struct sockaddr *from_addr)
{
	...
	if (r > 0) {
		...
		ACT_LOG_ID_DBG(ALF_STR_handle_request__NO_CONTENTFORMAT_GIV, 0);
		format = LWM2M_FORMAT_PLAIN_TEXT;
	}
	...
	if (r > 0) {
		...
		ACT_LOG_ID_DBG(ALF_STR_handle_request__NO_ACCEPT_OPTION_GIV, 0);
	...
	case LWM2M_OP_READ:
		if (observe == 0) {
			...
			if (token) {
				...
				if (r < 0) {
					ACT_LOG_ID_ERR(ALF_STR_handle_request__ADD_OBSERVE_ERROR_D, 1, r);
				}
			} else {
				ACT_LOG_ID_ERR(ALF_STR_handle_request__OBSERVE_REQUEST_MISS, 0);
			}
		} else if (observe == 1) {
			...
			if (r < 0) {
				ACT_LOG_ID_ERR(ALF_STR_handle_request__REMOVE_OBSERER_ERROR, 1, r);
			}
		}
	...
	default:
		ACT_LOG_ID_ERR(ALF_STR_handle_request__UNKNOWN_OPERATION_U, 1, context.operation);
		return -EINVAL;
	}
	...
	if (r == 0) {
		...
		if (out.outlen > 0) {
			ACT_LOG_ID_DBG(ALF_STR_handle_request__REPLYING_WITH_U_BYTE, 1, out.outlen);
			zoap_packet_set_used(out.out_zpkt, out.outlen);
		} else {
			ACT_LOG_ID_DBG(ALF_STR_handle_request__NO_DATA_IN_REPLY, 0);
		}
	} else {
		if (r == -ENOENT) {
						  ...
						  struct sockaddr *))
{
	...
	if (!udp_hdr) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_udp_receive__INVALID_UDP_DATA, 0);
		return;
	}
	...
	if (r < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_udp_receive__INVALID_DATA_RECEIVE, 1, r);
		goto cleanup;
	}
	...
	if (!reply) {
		...
		if (udp_request_handler &&
			...
			r = udp_request_handler(&response, &response2,
						&from_addr);
			if (r < 0) {
				ACT_LOG_ID_ERR(ALF_STR_lwm2m_udp_receive__REQUEST_HANDLER_ERRO, 1, r);
			} else {
			...
			ACT_LOG_ID_ERR(ALF_STR_lwm2m_udp_receive__NO_HANDLER_FOR_RESPO, 0);
		}
	} else {
		/*
		...
		if (handle_separate_response && !tkl &&
			...
			ACT_LOG_ID_DBG(ALF_STR_lwm2m_udp_receive__SEPARATED_RESPONSE_N, 0);
		} else {
			ACT_LOG_ID_DBG(ALF_STR_lwm2m_udp_receive__REPLY_08X_HANDLED_AN, 1, reply);
			zoap_reply_clear(reply);
		}
	}
				   ...
				   const struct sockaddr *from)
{
	...
	if (type == ZOAP_TYPE_RESET) {
		if (reply->tkl > 0) {
			...
			if (ret) {
				ACT_LOG_ID_ERR(ALF_STR_notify_message_reply_cb__REMOVE_OBSERER_ERROR, 1, ret);
			}
		} else {
			ACT_LOG_ID_ERR(ALF_STR_notify_message_reply_cb__NOTIFY_REPLY_MISSING, 0);
		}
	}
				   ...
				   bool manual_trigger)
{
	...
	ret = zoap_add_option_int(out.out_zpkt, ZOAP_OPTION_OBSERVE,
				  obs->counter);
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_generate_notify_message__OBSERVE_OPTION_ERROR, 1, ret);
		goto cleanup;
	}
	...
	ret = zoap_add_option_int(out.out_zpkt, ZOAP_OPTION_CONTENT_FORMAT,
				  obs->format);
	if (ret > 0) {
		ACT_LOG_ID_ERR(ALF_STR_generate_notify_message__ERROR_SETTING_CONTEN, 1, ret);
		goto cleanup;
	}
	...
	if (ret == 0) {
		if (out.outlen > 0) {
			...
			ACT_LOG_ID_DBG(ALF_STR_generate_notify_message__NO_DATA_IN_REPLY, 0);
		}
	} else {
		ACT_LOG_ID_ERR(ALF_STR_generate_notify_message__ERROR_IN_MULTIFORMAT, 1, ret);
		goto cleanup;
	}
	...
	if (!reply) {
		ACT_LOG_ID_ERR(ALF_STR_generate_notify_message__NO_RESOURCES_FOR_WAI, 0);
		ret = -ENOMEM;
		goto cleanup;
	}
	...
	if (ret < 0) {
		ACT_LOG_ID_ERR(ALF_STR_generate_notify_message__ERROR_SENDING_LWM2M_, 1, ret);
		goto cleanup;
	}
	ACT_LOG_ID_DBG(ALF_STR_generate_notify_message__NOTIFY_MSG_SENT, 0);
...
static int lwm2m_engine_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_lwm2m_engine_init__LWM2M_ENGINE_THREAD_, 0);
	return 0;
}
