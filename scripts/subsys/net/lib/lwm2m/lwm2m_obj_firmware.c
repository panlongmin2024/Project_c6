...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_OBJ_FIRMWARE
#include <net/zoap.h>
#include <string.h>
#include <init.h>
...
{
	ACT_LOG_ID_DBG(ALF_STR_package_write_cb__PACKAGE_WRITE, 0);
	if (write_cb) {
...
static struct lwm2m_engine_obj_inst *firmware_create(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_firmware_create__CREATE_LWM2M_FIRMWAR, 1, obj_inst_id);
	return &inst;
}
...
static int lwm2m_firmware_init(struct device *dev)
{
	...
	if (ret < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lwm2m_firmware_init__CREATE_LWM2M_INSTANC, 1, ret);
	}
