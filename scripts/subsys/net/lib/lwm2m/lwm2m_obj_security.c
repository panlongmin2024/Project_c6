...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_OBJ_SECURITY
#include <stdint.h>
#include <init.h>
...
static struct lwm2m_engine_obj_inst *security_create(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_security_create__CREATE_LWM2M_SECURIT, 1, obj_inst_id);
	return &inst[index];
}
...
static int lwm2m_security_init(struct device *dev)
{
	...
	if (ret < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_security_init__CREATE_LWM2M_SECURIT, 1, ret);
	}
