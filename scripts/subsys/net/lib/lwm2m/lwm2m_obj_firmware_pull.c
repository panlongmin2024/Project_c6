...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_OBJ_FIRMWARE_PULL
			    ...
			    zoap_reply_t reply_cb)
{
	...
	ret = zoap_add_option(&request, ZOAP_OPTION_URI_PATH,
			"1", sizeof("1") - 1);
	if (ret < 0) {
		ACT_LOG_ID_ERR(ALF_STR_transfer_request__ERROR_ADDING_URI_QUE, 0);
		goto cleanup;
	}

	ret = zoap_add_block2_option(&request, ctx);
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_transfer_request__UNABLE_TO_ADD_BLOCK2, 0);
		goto cleanup;
	}
	...
	if (reply_cb) {
		...
		if (!reply) {
			ACT_LOG_ID_ERR(ALF_STR_transfer_request__NO_RESOURCES_FOR_WAI, 0);
			ret = -ENOMEM;
			goto cleanup;
		}
			      ...
			      const struct sockaddr *from)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_do_firmware_transfer_reply_cb__TRANSFER_REPLY, 0);

	ret = zoap_update_from_block(check_response, &firmware_block_ctx);
	if (ret < 0) {
		ACT_LOG_ID_ERR(ALF_STR_do_firmware_transfer_reply_cb__ERROR_FROM_BLOCK_UPD, 1, ret);
		return ret;
	}
...
static void firmware_transfer(struct k_work *work)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_firmware_transfer__ATTACHED_TO_PORT_D, 1, port);
