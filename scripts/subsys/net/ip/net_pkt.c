...
#define ACT_LOG_MODEL_ID ALF_MODEL_NET_PKT
/** @file
...
#endif /* CONFIG_NET_DEBUG_NET_PKT */
{
	...
	if (k_is_in_isr()) {
		...
		if (pool == &inner_bufs) {
			...
			if (!frag) {
				ACT_LOG_ID_INF(ALF_STR_net_pkt_get_reserve_data_debug__CANT_ALLOC_FROM_INNE, 0);
...
void net_pkt_print_info(u32_t flag)
{
	...
	ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__FRAGMENT_LENGTH_D_BY, 1, CONFIG_NET_BUF_DATA_SIZE);
	ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__NETWORK_BUFFER_POOLS, 0);

#if defined(CONFIG_NET_DEBUG_NET_PKT)
	ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__ADDRESSTTSIZETCOUNTT, 0);
	...
	if (flag) {
		ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__NPKT_FROM_RXTXN, 0);
		for (i = 0; i < MAX_NET_PKT_ALLOCS; i++) {
		...
		ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__NFRAG_FROM_RXTXSHARE, 0);
		for (i = 0; i < MAX_NET_PKT_ALLOCS; i++) {
			if ((!net_pkt_allocs[i].in_use) || net_pkt_allocs[i].is_pkt) {
				continue;
			}
		...
		ACT_LOG_ID_INF(ALF_STR_net_pkt_print_info__N, 0);
	}
}
...
bool net_nbuf_add_dynamic_frag(void)
{
	...
	if (add_frag_flag) {
		ACT_LOG_ID_INF(ALF_STR_net_nbuf_add_dynamic_frag__DYNAMIC_FRAG_ALREADY, 0);
		return false;
	}
	...
	ACT_LOG_ID_INF(ALF_STR_net_nbuf_add_dynamic_frag__ADD_DYNAMIC_FRAGDN, 1, dynamic_frag_cnt);
	return true;
}
...
bool net_nbuf_release_dynamic_frag(void)
{
	...
	if (!add_frag_flag) {
		ACT_LOG_ID_INF(ALF_STR_net_nbuf_release_dynamic_frag__DYNAMIC_FRAG_ALREADY, 0);
		return false;
	}
	...
	ACT_LOG_ID_INF(ALF_STR_net_nbuf_release_dynamic_frag__ALL_DYNAMIC_FRAG_REL, 0);
