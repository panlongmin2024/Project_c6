...
sdfs_bin:
	...
	$(Q)if [ "X$(CONFIG_MUSIC_EXTERNAL_EFFECT)" != "X" ] ; then \
		if [ -d $(PROJECT_BASE)/dsp_bin/external ]; then \
			echo "Copy $(PROJECT_BASE)/dsp_bin/external/*.dsp"; \
			find $(PROJECT_BASE)/dsp_bin/external/ -type f -name "*.dsp" | xargs -i cp {} ${OUTDIR}/sdfs/; \
		fi \
	else \
		if [ -d $(PROJECT_BASE)/dsp_bin/default ]; then \
			echo "Copy $(PROJECT_BASE)/dsp_bin/default/*"; \
			find  $(PROJECT_BASE)/dsp_bin/default/ -type f -name "*.dsp" | xargs -i cp {}  ${OUTDIR}/sdfs/; \
		fi \
	fi;
...
firmware: nvram_bin datafs_bin sdfs_bin
	...
	$(Q)if [ "$(CONFIG_SOC_SECURE_BOOT)" = "y" ]; then	\
		$(PYTHON) -B $(srctree)/scripts/support/actions/build_firmware.py \
