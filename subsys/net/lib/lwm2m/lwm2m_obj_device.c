...
#define ACT_LOG_MODEL_ID ALF_MODEL_LWM2M_OBJ_DEVICE
#include <string.h>
#include <stdio.h>
#include <init.h>
...
{
	ACT_LOG_ID_DBG(ALF_STR_reboot_cb__REBOOT, 0);
	return -EPERM;
}

static int factory_default_cb(u16_t obj_inst_id)
{
	ACT_LOG_ID_DBG(ALF_STR_factory_default_cb__FACTORY_DEFAULT, 0);
	return -EPERM;
}
				 ...
				 bool last_block, size_t total_size)
{
	...
	ACT_LOG_ID_ERR(ALF_STR_current_time_post_write_cb__UNKNOWN_SIZE_U, 1, data_len);
	return 0;
}
...
int lwm2m_device_remove_pwrsrc(int index)
{
	if (index < 0 || index >= DEVICE_PWRSRC_MAX) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_remove_pwrsrc__INDEX_IS_OUT_OF_RANG, 1, index);
		return -EINVAL;
	}

	if (pwrsrc_available[index] < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_remove_pwrsrc__POWER_SOURCE_INDEX_D, 1, index);
		return -EINVAL;
	}
...
{
	if (index < 0 || index >= DEVICE_PWRSRC_MAX) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_set_pwrsrc_voltage_mv__INDEX_IS_OUT_OF_RANG, 1, index);
		return -EINVAL;
	}

	if (pwrsrc_available[index] < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_set_pwrsrc_voltage_mv__POWER_SOURCE_INDEX_D, 1, index);
		return -EINVAL;
	}
...
{
	if (index < 0 || index >= DEVICE_PWRSRC_MAX) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_set_pwrsrc_current_ma__INDEX_IS_OUT_OF_RANG, 1, index);
		return -EINVAL;
	}

	if (pwrsrc_available[index] < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lwm2m_device_set_pwrsrc_current_ma__POWER_SOURCE_INDEX_D, 1, index);
		return -EINVAL;
	}
...
static struct lwm2m_engine_obj_inst *device_create(u16_t obj_inst_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_device_create__CREATE_LWM2M_DEVICE_, 1, obj_inst_id);
	return &inst;
}
...
static int lwm2m_device_init(struct device *dev)
{
	...
	if (ret < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lwm2m_device_init__CREATE_LWM2M_INSTANC, 1, ret);
	}
