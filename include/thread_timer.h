...
/**
 * @brief Start a thread timer in specific thread.
 *
 * This routine starts a thread timer.
 *
 * Attempting to start a thread timer that is already running is permitted.
 ...
 * @param duration  Initial timer duration (in milliseconds).
 * @param period    Timer period (in milliseconds).
 * @param period    specific thread priority
 *
 * @return N/A
 */
extern void thread_timer_start_prio(struct thread_timer *ttimer, s32_t duration, s32_t period, k_tid_t tid);
