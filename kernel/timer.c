...
#define ACT_LOG_MODEL_ID ALF_MODEL_TIMER
...
#ifdef CONFIG_IRQ_LATENCY_STAT
	exc_time = k_cycle_get_32();
	/* invoke timer expiry function */
	if (timer->expiry_fn) {
		timer->expiry_fn(timer);
	}
	if(k_cycle_get_32() - exc_time > CONFIG_IRQ_LATENCY_STAT_TIME_US){
		printk("timer expiry func%p run too long (%dus)\n", timer->expiry_fn, (k_cycle_get_32() - exc_time)/24);
	}
#else
	/* invoke timer expiry function */
	if (timer->expiry_fn) {
