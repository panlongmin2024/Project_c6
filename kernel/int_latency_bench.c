...
#define ACT_LOG_MODEL_ID ALF_MODEL_INT_LATENCY_BENCH
...
void int_latency_show(void)
{
	...
	if (!int_latency_bench_ready) {
		ACT_LOG_ID_INF(ALF_STR_int_latency_show__ERROR_INT_LATENCY_IN, 0);
		return;
	}
	...
	if (int_locked_latency_min != ULONG_MAX) {
		...
		ACT_LOG_ID_INF(ALF_STR_int_latency_show__INTERRUPTS_WERE_NOT_, 0);
	}
	/*
