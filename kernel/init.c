...
#define ACT_LOG_MODEL_ID ALF_MODEL_INIT
...
struct _k_thread_stack_element  __attribute__((section (".noinit"))) __aligned(STACK_ALIGN) _idle_stack[CONFIG_IDLE_STACK_SIZE];
...
//struct _k_thread_stack_element __btc_host_bss __aligned(STACK_ALIGN) _interrupt_stack[CONFIG_ISR_STACK_SIZE];
struct _k_thread_stack_element __aligned(STACK_ALIGN) _interrupt_stack[CONFIG_ISR_STACK_SIZE];
...
extern struct _k_thread_stack_element  __attribute__((section (".noinit")))  __aligned(STACK_ALIGN)
	sys_work_q_stack[CONFIG_SYSTEM_WORKQUEUE_STACK_SIZE];
...
void _bss_zero(void)
{
	...
	ROM_MEMCPY((void *)&g_boot_info, (const void *)boot_info_addr, sizeof(g_boot_info));
#endif

	ROM_MEMSET(&__bss_start, 0,
		 ((u32_t) &__bss_end - (u32_t) &__bss_start));
#ifdef CONFIG_APPLICATION_MEMORY
	ROM_MEMSET(&__app_bss_start, 0,
		 ((u32_t) &__app_bss_end - (u32_t) &__app_bss_start));
#endif
	extern char __btcon_host_buf_start[];
	extern char __btcon_host_buf_end[];
	extern char __btcon_host_bss_start[];
	extern char __btcon_host_bss_end[];
	ROM_MEMSET(&__btcon_host_buf_start, 0,
		((u32_t) &__btcon_host_buf_end - (u32_t) &__btcon_host_buf_start));
	ROM_MEMSET(&__btcon_host_bss_start, 0,
		((u32_t) &__btcon_host_bss_end - (u32_t) &__btcon_host_bss_start));
}
...
{
	ROM_MEMCPY(&__data_ram_start, &__data_rom_start,
		 ((u32_t) &__data_ram_end - (u32_t) &__data_ram_start));
#ifdef CONFIG_APPLICATION_MEMORY
	ROM_MEMCPY(&__app_data_ram_start, &__app_data_rom_start,
		 ((u32_t) &__app_data_ram_end - (u32_t) &__app_data_ram_start));
#endif

#ifdef CONFIG_BTCONRAMFUNC_MEMORY
	ROM_MEMCPY(&__btcon_ramfunc_start, &__btcon_ramfuc_rom_start,
		 ((u32_t) &__btcon_ramfunc_end - (u32_t) &__btcon_ramfunc_start));
#endif

#ifdef CONFIG_RAMFUNCTION_MEMORY
	ROM_MEMCPY(&_image_text_ramfunc_start, &_image_text_ramfunc_rom_start,
		 ((u32_t) &_image_text_ramfunc_end - (u32_t) &_image_text_ramfunc_start));
#endif
}
#endif
...
static void prepare_multithreading(struct k_thread *dummy_thread)
{
	...
	dummy_thread->base.sched_locked = -1;
