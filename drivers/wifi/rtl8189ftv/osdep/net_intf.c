...
#define ACT_LOG_MODEL_ID ALF_MODEL_NET_INTF
...
int netif_tx(int idx, uint8_t *data, uint32_t len)
{
	...
	if(idx == -1){
		ACT_LOG_ID_ERR(ALF_STR_netif_tx__NETIF_IS_DOWN, 0);
		return ret;
	}
	...
	if (true) {
		...
		if (skb == NULL) {
			ACT_LOG_ID_ERR(ALF_STR_netif_tx__RLTK_WLAN_ALLOC_SKBF, 0);
			goto exit;
		}
		skb->data = data;
		...
		if (skb == NULL) {
			ACT_LOG_ID_ERR(ALF_STR_netif_tx__RLTK_WLAN_ALLOC_SKB_, 1, len);
			goto exit;
		}

		//ACT_LOG_ID_INF(ALF_STR_netif_tx__NETIF_TX_LENDN, 1, len);
...
void netif_rx(int idx, unsigned int len)
{
	...
	if(!rltk_wlan_running(idx)) {
		ACT_LOG_ID_ERR(ALF_STR_netif_rx__WIFI_NOT_RUNNINGN, 0);
		return;
	}
	...
	if (!frag) {
		ACT_LOG_ID_ERR(ALF_STR_netif_rx__COULD_NOT_ALLOCATE_D, 0);
		return;
	}
	...
	if (!pkt) {
		ACT_LOG_ID_ERR(ALF_STR_netif_rx__CANNOT_ALLOCATE_PBUF, 0);
	...
	//ACT_LOG_ID_INF(ALF_STR_netif_rx__NETIF_RX_LENDN, 1, len);
