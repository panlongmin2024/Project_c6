...
#define ACT_LOG_MODEL_ID ALF_MODEL_ZEPHYR_SERVICE
...
void wifi_mem_debug_print(void)
{
	...
	ACT_LOG_ID_INF(ALF_STR_wifi_mem_debug_print__BTC_MEM_MAX_MALLOC_C, 2, btc_max_malloc_cut, btc_cur_malloc_cut);
	ACT_LOG_ID_INF(ALF_STR_wifi_mem_debug_print__BTC_BTC_MAX_MEMORY_U, 2, btc_max_memory_used, btc_cur_memory_used);

	for (i=0; i<BTC_MEM_DEBUG_MAX; i++) {
		if (btc_mem_debug[i][0] != 0) {
			ACT_LOG_ID_INF(ALF_STR_wifi_mem_debug_print__UNFREE_MEM_ADDR0XX_S, 2, btc_mem_debug[i][0], btc_mem_debug[i][1]);
			flag = 1;
		}
	}

	if (!flag) {
		ACT_LOG_ID_INF(ALF_STR_wifi_mem_debug_print__ALL_MEM_FREEN, 0);
	}
}
#else
...
static char wifi_large_mem_0[4016] __aligned(4);
static char wifi_large_mem_1[5728] __aligned(4);
...
void *wifi_mem_malloc(unsigned int num_bytes)
{
	void *p;

	if (num_bytes > 5728) {
		printk("ERR: %s, mem %d > 5712\n", __func__, num_bytes);
		//dump_stack();
		return NULL;
	} else if (num_bytes == 4016) {
		printk("%s, large mem:%d\n", __func__, num_bytes);
		if (large_mem_0_used) {
			ACT_LOG_ID_INF(ALF_STR_wifi_mem_malloc__ERR_MULTI_USED_LARGE, 0);
			//dump_stack();
			return NULL;
		}
		large_mem_0_used = 1;
		return wifi_large_mem_0;
	} else if (num_bytes == 5728) {///for Adapter
		printk("%s, large mem:%d\n", __func__, num_bytes);
		if (large_mem_1_used) {
			ACT_LOG_ID_INF(ALF_STR_wifi_mem_malloc__ERR_MULTI_USED_LARGE, 0);
			//dump_stack();
		...
		large_mem_1_used = 1;
		return wifi_large_mem_1;
	} else if (num_bytes == 2048) {
		/* Only for wifi sdio rx data */
		if (large_mem_2_used) {
			ACT_LOG_ID_INF(ALF_STR_wifi_mem_malloc__ERR_MULTI_USED_LARGE, 0);
			//dump_stack();
			return NULL;
		...
		large_mem_2_used = 1;
		return wifi_large_mem_2;
	} else if (num_bytes > 1536) {
		/* Wifi receive data packet large than 1640, not for me, just drop */
		printk("INF: %s, mem %d > 1536\n", __func__, num_bytes);
		return NULL;
	}

	p = mem_malloc(num_bytes);
	if (p) {
		wifi_mem_debug_malloc(p, num_bytes);
	}
...
{
	if (wifi_large_mem_1 == ptr) {
		large_mem_1_used = 0;
		return;
	}

	if (wifi_large_mem_2 == ptr) {
		large_mem_2_used = 0;
		return;
	}
...
static void _zephyr_init_sema(_sema *sema, int init_val)
{
	*sema = wifi_mem_malloc(sizeof(struct k_sem));
	...
	if (*sema == NULL) {
		printk("%s, %d, Can't malloc memory!\n", __func__, __LINE__);
	} else {
	...
	u32  stack_size, u32 priority, thread_func_t func, void *thctx)
{
	...
	if (priority == 2) {
		...
		ACT_LOG_ID_INF(ALF_STR__zephyr_create_task__TODO_MORE_THREAD_WAI, 0);
		return 0;
	}
...
{
	if (!ptask->task){
		ACT_LOG_ID_INF(ALF_STR__zephyr_delete_task___FREERTOS_DELETE_TAS, 0);
		return;
	}
	...
	ACT_LOG_ID_INF(ALF_STR__zephyr_delete_task__NWARNING__ZEPHYR_DEL, 0);
}
