...
#define ACT_LOG_MODEL_ID ALF_MODEL_SSV6030P
#include <stdio.h>
#endif
...
static int ssv6030p_send(struct net_if *iface, struct net_pkt *pkt)
{
	...
	//ACT_LOG_ID_DBG(ALF_STR_ssv6030p_send__TXDN, 1, len);
...
int ssv6030p_recv_cb(void *data, size_t len, uint32_t hdr_offse)
{
	...
	if (!pkt) {
		ACT_LOG_ID_WRN(ALF_STR_ssv6030p_recv_cb__CANT_NET_PKT_GET_RES, 0);
		goto free_data;
	}
...
static int ssv6030p_init(struct device *dev)
{
    ...
    vPortInit();
    ACT_LOG_ID_INF(ALF_STR_ssv6030p_init___USE_HEAP_5_MEMORY_R, 0);
#else
    ACT_LOG_ID_INF(ALF_STR_ssv6030p_init___USE_ZERPHY_MEMORY_R, 0);
#endif

	ssv_wifi_init_done = false;

	if (0 != ssv6xxx_dev_init(0)) {
		ACT_LOG_ID_INF(ALF_STR_ssv6030p_init__SSV6030P__NO_READYRN, 0);
		return -1;
	}
	...
	ACT_LOG_ID_INF(ALF_STR_ssv6030p_init__SSV6030P_READY_RN, 0);
