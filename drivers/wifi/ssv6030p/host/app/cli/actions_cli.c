...
#define ACT_LOG_MODEL_ID ALF_MODEL_ACTIONS_CLI
...
static evt_handler app_cb_handle = NULL;
...
void wifi_exit_ps(bool exit)
{
	...
	if (wifi_power_state == WIFI_POWER_SAVE) {
		//ACT_LOG_ID_INF(ALF_STR_wifi_exit_ps__WW_ENTERN, 0);
		wifi_power_state = WIFI_POWER_WAKING;
		ssv6xxx_wifi_wakeup();
		//ACT_LOG_ID_INF(ALF_STR_wifi_exit_ps__WW_EXITN, 0);
	} else if (wifi_power_state == WIFI_POWER_SAVING) {
		ACT_LOG_ID_INF(ALF_STR_wifi_exit_ps__WIFI_STILL_IN_WIFI_P, 0);
	}
...
static void wifi_power_save_timeout(struct k_work *work)
{
	...
	if ((wifi_power_state == WIFI_POWER_WAKE) && (app_cb_handle == NULL) &&
		...
		if ((wifi_power_state == WIFI_POWER_WAKE) &&
			...
			wifi_PS_cnt = 0;
			//ACT_LOG_ID_INF(ALF_STR_wifi_power_save_timeout__WP_ENTERN, 0);
			wifi_power_state = WIFI_POWER_SAVING;
			wifi_enter_ps();
			//ACT_LOG_ID_INF(ALF_STR_wifi_power_save_timeout__WP_EXITN, 0);
		}
...
void act_cmd_leave(s32 argc, char *argv[])
{
    ...
    if (ret != 0)
    {
	    ACT_LOG_ID_INF(ALF_STR_act_cmd_leave__NETMGR_WIFI_LEAVE_FA, 0);
    }
}
...
static void wifi_event_cb_handler(u32 evt_id, void *data, s32 len)
{
	...
	switch (evt_id) {
		...
		case SOC_EVT_JOIN_RESULT:
			...
			{
				ACT_LOG_ID_INF(ALF_STR_wifi_event_cb_handler__JOIN_FAILURERN, 0);
			}
			if (evt_result->u.join.status_code == 2)
			{
				ACT_LOG_ID_INF(ALF_STR_wifi_event_cb_handler__JOIN_FAILURE_BECAUSE, 0);
			}
		...
		case SOC_EVT_SCAN_DONE:
			...
			ACT_LOG_ID_INF(ALF_STR_wifi_event_cb_handler__SCAN_DOWNN, 0);
#if WIFI_CONNECT_JOIN_ASYNC
			join_scan_done_cb();
#endif
			break;
		...
		case SOC_EVT_PS_SETUP_OK:
			...
			//ACT_LOG_ID_INF(ALF_STR_wifi_event_cb_handler__WP_CBN, 0);
#endif
			break;
		case SOC_EVT_PS_WAKENED:
			...
			wifi_PS_cnt = 0;
			wifi_power_state = WIFI_POWER_WAKE;
			//ACT_LOG_ID_INF(ALF_STR_wifi_event_cb_handler__WW_CBN, 0);
#endif
			break;
		default:
...
*/
void reg_wifi_evt_cb(evt_handler cbhandle)
{
	k_mutex_lock(&cb_handle_mutex, K_FOREVER);
