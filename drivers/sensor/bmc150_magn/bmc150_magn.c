...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMC150_MAGN
				    ...
				    enum sensor_channel chan)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
			   ...
			   sizeof(values)) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
				...
				const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		...
		if (data->max_odr < val->val1) {
			ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_attr_set__NOT_SUPPORTED_WITH_C, 0);
			return -ENOTSUP;
		}
...
static int bmc150_magn_init_chip(struct device *dev)
{
	...
	if (bmc150_magn_set_power_mode(dev, BMC150_MAGN_POWER_MODE_SUSPEND, 0)
				       < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__FAILED_TO_BRING_UP_D, 0);
		return -EIO;
	}

	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
			      BMC150_MAGN_REG_CHIP_ID, &chip_id) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__FAILED_READING_CHIP_, 0);
		goto err_poweroff;
	}
	if (chip_id != BMC150_MAGN_CHIP_ID_VAL) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		goto err_poweroff;
	}
	ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__CHIP_ID_0XX, 1, chip_id);
	...
	if (bmc150_magn_set_power_mode(dev, BMC150_MAGN_POWER_MODE_NORMAL, 1)
				       < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__FAILED_TO_POWER_ON_D, 0);
		goto err_poweroff;
	}
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
			   ...
			   sizeof(data->tregs)) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_chip__FAILED_TO_READ_TRIM_, 0);
		goto err_poweroff;
	}
...
static int bmc150_magn_init(struct device *dev)
{
	...
	if (bmc150_magn_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
