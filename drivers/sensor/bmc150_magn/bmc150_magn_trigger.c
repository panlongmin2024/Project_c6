...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMC150_MAGN_TRIGGER
			    ...
			    sensor_trigger_handler_t handler)
{
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte(data->i2c_master,
					...
					< 0) {
			ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_trigger_set__FAILED_TO_SET_DRDY_I, 0);
			return -EIO;
		}
...
static void bmc150_magn_thread_main(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		while (i2c_reg_read_byte(data->i2c_master,
					 ...
					 &reg_val) < 0) {
			ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_thread_main__FAILED_TO_CLEAR_DATA, 0);
		}
...
int bmc150_magn_init_interrupt(struct device *dev)
{
	...
	if (bmc150_magn_set_drdy_polarity(dev, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_interrupt__FAILED_TO_SET_DR_POL, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_interrupt__FAILED_TO_SET_DATA_R, 0);
		return -EIO;
	}
#endif
	...
	if (gpio_add_callback(data->gpio_drdy, &data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmc150_magn_init_interrupt__FAILED_TO_SET_GPIO_C, 0);
		return -EIO;
	}
