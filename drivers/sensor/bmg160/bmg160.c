...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMG160
...
int bmg160_init(struct device *dev)
{
	...
	if (!bmg160->i2c) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__I2C_MASTER_CONTROLLE, 0);
		return -EINVAL;
	}
	...
	if (bmg160_read_byte(dev, BMG160_REG_CHIPID, &chip_id) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (chip_id != BMG160_CHIP_ID) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__UNSUPPORTED_CHIP_DET, 1, chip_id);
		return -ENODEV;
	}
	...
	if (bmg160_write_byte(dev, BMG160_REG_RANGE,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__FAILED_TO_SET_RANGE, 0);
		return -EIO;
	}
	...
	if (bmg160_write_byte(dev, BMG160_REG_BW, BMG160_DEFAULT_ODR) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__FAILED_TO_SET_SAMPLI, 0);
		return -EIO;
	}

	/* disable interrupts */
	if (bmg160_write_byte(dev, BMG160_REG_INT_EN0, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_init__FAILED_TO_DISABLE_AL, 0);
		return -EIO;
	}
