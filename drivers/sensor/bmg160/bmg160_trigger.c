...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMG160_TRIGGER
...
int bmg160_trigger_init(struct device *dev)
{
	...
	if (bmg160_write_byte(dev, BMG160_REG_INT_EN1, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_trigger_init__FAILED_TO_SELECT_INT, 0);
		return -EIO;
	}

	/* set interrupt mode to non-latched */
	if (bmg160_write_byte(dev, BMG160_REG_INT_RST_LATCH, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmg160_trigger_init__FAILED_TO_SET_THE_IN, 0);
		return -EIO;
	}
	...
	if (bmg160_write_byte(dev, BMG160_REG_INT_MAP0,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmg160_trigger_init__UNABLE_TO_MAP_INTERR, 0);
		return -EIO;
	}
