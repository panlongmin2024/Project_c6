...
#define ACT_LOG_MODEL_ID ALF_MODEL_ADXL362
			   ...
			   u8_t register_address, u8_t count)
{
	...
	if (ret) {
		ACT_LOG_ID_DBG(ALF_STR_adxl362_set_reg__SPI_SLAVE_SELECT_FAI, 1, ret);
		return ret;
	}
	...
	ret = spi_transceive(adxl362_data->spi, buffer, count + 2,
			     buffer, count + 2);
	if (ret) {
		ACT_LOG_ID_DBG(ALF_STR_adxl362_set_reg__SPI_TRANSCEIVE_FAIL_, 1, ret);
		return ret;
	}
			   ...
			   u8_t register_address, u8_t count)
{
	...
	ret = spi_slave_select(adxl362_data->spi,
			       adxl362_data->spi_slave);
	if (ret) {
		ACT_LOG_ID_DBG(ALF_STR_adxl362_get_reg__SPI_SLAVE_SELECT_FAI, 1, ret);
		return ret;
	}
	...
	ret = spi_transceive(adxl362_data->spi, buffer, count + 2,
			     buffer, count + 2);
	if (ret) {
		ACT_LOG_ID_DBG(ALF_STR_adxl362_get_reg__SPI_TRANSCEIVE_FAIL_, 1, ret);
		return ret;
	}
			     ...
			     const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_FULL_SCALE:
	{
		...
		if (range_reg < 0) {
			ACT_LOG_ID_DBG(ALF_STR_axl362_acc_config__INVALID_RANGE_REQUES, 0);
			return -ENOTSUP;
		}
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
	{
		...
		out_rate = adxl362_freq_to_odr_val(val->val1,
						   val->val2 / 1000);
		if (out_rate < 0) {
			ACT_LOG_ID_DBG(ALF_STR_axl362_acc_config__INVALID_OUTPUT_RATE, 0);
			return -ENOTSUP;
		}
...
#endif
	default:
		ACT_LOG_ID_DBG(ALF_STR_axl362_acc_config__ACCEL_ATTRIBUTE_NOT_, 0);
		return -ENOTSUP;
	}
		    ...
		    enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	default:
		ACT_LOG_ID_DBG(ALF_STR_adxl362_attr_set__ATTR_SET_NOT_SUPPORT, 0);
		return -ENOTSUP;
	}
