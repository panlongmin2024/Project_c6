...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMM150
...
static int bmm150_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, config->i2c_slave_addr,
			   ...
			   sizeof(values)) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
			   ...
			   const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		...
		if (data->max_odr < val->val1) {
			ACT_LOG_ID_ERR(ALF_STR_bmm150_attr_set__NOT_SUPPORTED_WITH_C, 0);
			return -ENOTSUP;
		}
...
static int bmm150_init_chip(struct device *dev)
{
	...
	if (bmm150_set_power_mode(dev, BMM150_POWER_MODE_NORMAL, 0) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init_chip__FAILED_TO_BRING_UP_D, 0);
		return -EIO;
	}
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_slave_addr,
		...
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init_chip__FAILED_READING_CHIP_, 0);
		goto err_poweroff;
	}

	if (chip_id != BMM150_CHIP_ID_VAL) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		goto err_poweroff;
	}
	...
	if (bmm150_set_power_mode(dev, BMM150_POWER_MODE_NORMAL, 1)
	    < 0) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init_chip__FAILED_TO_POWER_ON_D, 0);
	}

	if (i2c_burst_read(data->i2c, config->i2c_slave_addr,
			   BMM150_REG_TRIM_START, (u8_t *)&data->tregs,
			   sizeof(data->tregs)) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init_chip__FAILED_TO_READ_TRIM_, 0);
		goto err_poweroff;
	}
...
static int bmm150_init(struct device *dev)
{
	...
	if (bmm150_init_chip(dev) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_bmm150_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
