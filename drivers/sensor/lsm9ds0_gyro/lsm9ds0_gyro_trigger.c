...
#define ACT_LOG_MODEL_ID ALF_MODEL_LSM9DS0_GYRO_TRIGGER
			     ...
			     sensor_trigger_handler_t handler)
{
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte(data->i2c_master,
					...
					< 0) {
			ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_trigger_set__FAILED_TO_SET_DRDY_I, 0);
			return -EIO;
		}
...
int lsm9ds0_gyro_init_interrupt(struct device *dev)
{
	...
	if (gpio_add_callback(data->gpio_drdy, &data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_interrupt__FAILED_TO_SET_GPIO_C, 0);
		return -EINVAL;
	}
