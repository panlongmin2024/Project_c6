...
#define ACT_LOG_MODEL_ID ALF_MODEL_LSM9DS0_GYRO
				     ...
				     enum sensor_channel chan)
{
	...
	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
				 ...
				 const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_FULL_SCALE:
		if (lsm9ds0_gyro_set_fs(dev, sensor_rad_to_degrees(val)) < 0) {
			ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_attr_set__FULLSCALE_VALUE_NOT_, 0);
			return -EIO;
		}
		break;
#endif
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		if (lsm9ds0_gyro_set_odr(dev, val->val1) < 0) {
			ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_attr_set__SAMPLING_FREQUENCY_V, 0);
			return -EIO;
		}
		break;
#endif
...
static int lsm9ds0_gyro_init_chip(struct device *dev)
{
	...
	if (lsm9ds0_gyro_power_ctrl(dev, 0, 0, 0, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_TO_POWER_OFF_, 0);
		return -EIO;
	}

	if (lsm9ds0_gyro_power_ctrl(dev, 1, 1, 1, 1) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_TO_POWER_ON_D, 0);
		return -EIO;
	}

	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
			      LSM9DS0_GYRO_REG_WHO_AM_I_G, &chip_id) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_READING_CHIP_, 0);
		goto err_poweroff;
	}
	if (chip_id != LSM9DS0_GYRO_VAL_WHO_AM_I_G) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		goto err_poweroff;
	}
	ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__CHIP_ID_0XX, 1, chip_id);

	if (lsm9ds0_gyro_set_fs_raw(dev, LSM9DS0_GYRO_DEFAULT_FULLSCALE) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_TO_SET_FULLSC, 0);
		goto err_poweroff;
	}

	if (lsm9ds0_gyro_set_odr_raw(dev, LSM9DS0_GYRO_DEFAULT_SAMPLING_RATE)
				     < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_TO_SET_SAMPLI, 0);
		goto err_poweroff;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
				...
				< 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init_chip__FAILED_TO_SET_BDU_AN, 0);
		goto err_poweroff;
	}
...
static int lsm9ds0_gyro_init(struct device *dev)
{
	...
	if (lsm9ds0_gyro_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm9ds0_gyro_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
