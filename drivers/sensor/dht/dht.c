...
#define ACT_LOG_MODEL_ID ALF_MODEL_DHT
...
static int dht_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (((buf[0] + buf[1] + buf[2] + buf[3]) & 0xFF) != buf[4]) {
		ACT_LOG_ID_DBG(ALF_STR_dht_sample_fetch__INVALID_CHECKSUM_IN_, 0);
		ret = -EIO;
	} else {
...
static int dht_init(struct device *dev)
{
	...
	if (drv_data->gpio == NULL) {
		ACT_LOG_ID_ERR(ALF_STR_dht_init__FAILED_TO_GET_GPIO_D, 0);
		return -EINVAL;
	}
