...
#define ACT_LOG_MODEL_ID ALF_MODEL_TMP007_TRIGGER
		    ...
		    const struct sensor_value *val)
{
	...
	if (tmp007_reg_write(drv_data, reg, value) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_tmp007_attr_set__FAILED_TO_SET_ATTRIB, 0);
		return -EIO;
	}
...
int tmp007_init_interrupt(struct device *dev)
{
	...
	if (tmp007_reg_update(drv_data, TMP007_REG_CONFIG,
		...
		ACT_LOG_ID_DBG(ALF_STR_tmp007_init_interrupt__FAILED_TO_ENABLE_INT, 0);
		return -EIO;
	}
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_tmp007_init_interrupt__FAILED_TO_SET_GPIO_C, 0);
		return -EIO;
	}
