...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMA280_TRIGGER
		    ...
		    const struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_SLOPE_TH) {
		...
		if (i2c_reg_write_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
				       ...
				       < 0) {
			ACT_LOG_ID_DBG(ALF_STR_bma280_attr_set__COULD_NOT_SET_SLOPE_, 0);
			return -EIO;
		}
...
static void bma280_thread_cb(void *arg)
{
	...
	if (status & BMA280_BIT_SLOPE_INT_STATUS &&
		...
		if (err < 0) {
			ACT_LOG_ID_DBG(ALF_STR_bma280_thread_cb__COULD_NOT_UPDATE_CLE, 0);
			return;
		}
	}
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
			...
			ACT_LOG_ID_DBG(ALF_STR_bma280_trigger_set__COULD_NOT_DISABLE_DA, 0);
			return -EIO;
		}
		...
		if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
			...
			ACT_LOG_ID_DBG(ALF_STR_bma280_trigger_set__COULD_NOT_ENABLE_DAT, 0);
			return -EIO;
		}
...
int bma280_init_interrupt(struct device *dev)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_init_interrupt__COULD_NOT_SET_LATCHE, 0);
		return -EIO;
	}
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bma280_init_interrupt__COULD_NOT_SET_GPIO_C, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_init_interrupt__COULD_NOT_MAP_DATA_R, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_init_interrupt__COULD_NOT_MAP_ANYMOT, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
				BMA280_REG_INT_EN_1,
				BMA280_BIT_DATA_EN, 0) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bma280_init_interrupt__COULD_NOT_DISABLE_DA, 0);
		return -EIO;
	}
