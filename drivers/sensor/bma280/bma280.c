...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMA280
...
static int bma280_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_sample_fetch__COULD_NOT_READ_ACCEL, 0);
		return -EIO;
	}
	...
	if (i2c_reg_read_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_sample_fetch__COULD_NOT_READ_TEMPE, 0);
		return -EIO;
	}
...
int bma280_init(struct device *dev)
{
	...
	if (i2c_reg_read_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_init__COULD_NOT_READ_CHIP_, 0);
		return -EIO;
	}

	if (id != BMA280_CHIP_ID) {
		ACT_LOG_ID_DBG(ALF_STR_bma280_init__UNEXPECTED_CHIP_ID_X, 1, id);
		return -EIO;
	}

	if (i2c_reg_write_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
			       BMA280_REG_PMU_BW, BMA280_PMU_BW) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bma280_init__COULD_NOT_SET_DATA_F, 0);
		return -EIO;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_bma280_init__COULD_NOT_SET_DATA_G, 0);
		return -EIO;
	}

#ifdef CONFIG_BMA280_TRIGGER
	if (bma280_init_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bma280_init__COULD_NOT_INITIALIZE, 0);
		return -EIO;
	}
#endif
