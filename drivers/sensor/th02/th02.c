...
#define ACT_LOG_MODEL_ID ALF_MODEL_TH02
...
static u8_t read8(struct device *dev, u8_t d)
{
	...
	if (i2c_reg_read_byte(dev, TH02_I2C_DEV_ID, d, &buf) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_read8__ERROR_READING_REGIST, 0);
	}
	return buf;
}
...
static int is_ready(struct device *dev)
{
	...
	if (i2c_reg_read_byte(dev, TH02_I2C_DEV_ID,
		...
		ACT_LOG_ID_ERR(ALF_STR_is_ready__ERROR_READING_STATUS, 0);
	}
...
static u16_t get_humi(struct device *dev)
{
	...
	if (i2c_reg_write_byte(dev, TH02_I2C_DEV_ID,
		...
		ACT_LOG_ID_ERR(ALF_STR_get_humi__ERROR_WRITING_REGIST, 0);
		return 0;
	}
	while (!is_ready(dev)) {
	}
...
u16_t get_temp(struct device *dev)
{
	...
	if (i2c_reg_write_byte(dev, TH02_I2C_DEV_ID,
		...
		ACT_LOG_ID_ERR(ALF_STR_get_temp__ERROR_WRITING_REGIST, 0);
		return 0;
	}
	while (!is_ready(dev)) {
	}
...
static int th02_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	ACT_LOG_ID_INF(ALF_STR_th02_sample_fetch__TEMP_U, 1, drv_data->t_sample);
	drv_data->rh_sample = get_humi(drv_data->i2c);
	ACT_LOG_ID_INF(ALF_STR_th02_sample_fetch__RH_U, 1, drv_data->rh_sample);
