...
#define ACT_LOG_MODEL_ID ALF_MODEL_HMC5883L
...
static int hmc5883l_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, HMC5883L_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_sample_fetch__FAILED_TO_FETCH_MEGN, 0);
		return -EIO;
	}
...
int hmc5883l_init(struct device *dev)
{
	...
	if (i2c_burst_read(drv_data->i2c, HMC5883L_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (id[0] != HMC5883L_CHIP_ID_A || id[1] != HMC5883L_CHIP_ID_B ||
	    id[2] != HMC5883L_CHIP_ID_C) {
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__INVALID_CHIP_ID, 0);
		return -EINVAL;
	}
	...
	if (idx == ARRAY_SIZE(hmc5883l_fs_strings)) {
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__INVALID_FULLSCALE_RA, 0);
		return -EINVAL;
	}
	...
	if (idx == ARRAY_SIZE(hmc5883l_odr_strings)) {
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__INVALID_ODR_VALUE, 0);
		return -EINVAL;
	}
	...
	if (i2c_burst_write(drv_data->i2c, HMC5883L_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__FAILED_TO_CONFIGURE_, 0);
		return -EIO;
	}

#ifdef CONFIG_HMC5883L_TRIGGER
	if (hmc5883l_init_interrupt(dev) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hmc5883l_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
