...
#define ACT_LOG_MODEL_ID ALF_MODEL_ISL29035_TRIGGER
		      ...
		      const struct sensor_value *val)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, ISL29035_I2C_ADDRESS,
	    ...
	    i2c_reg_write_byte(drv_data->i2c, ISL29035_I2C_ADDRESS,
			       msb_reg, raw_val >> 8) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_isl29035_attr_set__FAILED_TO_SET_ATTRIB, 0);
		return -EIO;
	}
...
int isl29035_init_interrupt(struct device *dev)
{
	...
	if (i2c_reg_update_byte(drv_data->i2c, ISL29035_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_isl29035_init_interrupt__FAILED_TO_SET_INTERR, 0);
		return -EIO;
	}
	...
	if (drv_data->gpio == NULL) {
		ACT_LOG_ID_DBG(ALF_STR_isl29035_init_interrupt__FAILED_TO_GET_GPIO_D, 0);
		return -EINVAL;
	}
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_isl29035_init_interrupt__FAILED_TO_SET_GPIO_C, 0);
		return -EIO;
	}
