...
#define ACT_LOG_MODEL_ID ALF_MODEL_AK8975
...
static int ak8975_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_AK8975_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_ak8975_sample_fetch__FAILED_TO_START_MEAS, 0);
		return -EIO;
	}
	...
	if (i2c_burst_read(drv_data->i2c, CONFIG_AK8975_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_ak8975_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int ak8975_read_adjustment_data(struct ak8975_data *drv_data)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_AK8975_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_ak8975_read_adjustment_data__FAILED_TO_SET_CHIP_I, 0);
		return -EIO;
	}

	if (i2c_burst_read(drv_data->i2c, CONFIG_AK8975_I2C_ADDR,
			   AK8975_REG_ADJ_DATA_START, buf, 3) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_ak8975_read_adjustment_data__FAILED_TO_READ_ADJUS, 0);
		return -EIO;
	}
...
int ak8975_init(struct device *dev)
{
	...
	if (i2c_reg_update_byte(drv_data->i2c, MPU9150_I2C_ADDR,
				...
				0) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_ak8975_init__FAILED_TO_WAKE_UP_MP, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(drv_data->i2c, MPU9150_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_ak8975_init__FAILED_TO_ENABLE_PAS, 0);
		return -EIO;
	}
#endif
	...
	if (i2c_reg_read_byte(drv_data->i2c, CONFIG_AK8975_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_ak8975_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (id != AK8975_CHIP_ID) {
		ACT_LOG_ID_ERR(ALF_STR_ak8975_init__INVALID_CHIP_ID, 0);
		return -EINVAL;
	}
