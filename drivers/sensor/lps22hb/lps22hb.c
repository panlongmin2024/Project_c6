...
#define ACT_LOG_MODEL_ID ALF_MODEL_LPS22HB
				...
				enum sensor_channel chan)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lps22hb_init_chip(struct device *dev)
{
	...
	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_init_chip__FAILED_READING_CHIP_, 0);
		goto err_poweroff;
	}

	if (chip_id != LPS22HB_VAL_WHO_AM_I) {
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		goto err_poweroff;
	}

	if (lps22hb_set_odr_raw(dev, LPS22HB_DEFAULT_SAMPLING_RATE) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_init_chip__FAILED_TO_SET_SAMPLI, 0);
		goto err_poweroff;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_init_chip__FAILED_TO_SET_BDU, 0);
		goto err_poweroff;
	}
...
static int lps22hb_init(struct device *dev)
{
	...
	if (lps22hb_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lps22hb_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
