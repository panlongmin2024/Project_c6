...
#define ACT_LOG_MODEL_ID ALF_MODEL_MPU6050
...
static int mpu6050_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_sample_fetch__FAILED_TO_READ_DATA_, 0);
		return -EIO;
	}
...
int mpu6050_init(struct device *dev)
{
	...
	if (i2c_reg_read_byte(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (id != MPU6050_CHIP_ID) {
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__INVALID_CHIP_ID, 0);
		return -EINVAL;
	}
	...
	if (i2c_reg_update_byte(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
				...
				0) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__FAILED_TO_WAKE_UP_CH, 0);
		return -EIO;
	}
	...
	if (i == 4) {
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__INVALID_VALUE_FOR_AC, 0);
		return -EINVAL;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__FAILED_TO_WRITE_ACCE, 0);
		return -EIO;
	}
	...
	if (i == 4) {
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__INVALID_VALUE_FOR_GY, 0);
		return -EINVAL;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_MPU6050_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_mpu6050_init__FAILED_TO_WRITE_GYRO, 0);
		return -EIO;
	}
	...
	if (mpu6050_init_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_mpu6050_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
