...
#define ACT_LOG_MODEL_ID ALF_MODEL_BMI160
			     ...
			     u8_t rx_buf_len)
{
	...
	if (spi_configure(bmi160->spi, &spi_cfg) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_transceive__CANNOT_CONFIGURE_SPI, 0);
		return -EIO;
	}

	if (spi_slave_select(bmi160->spi, dev_cfg->spi_slave) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_transceive__CANNOT_SELECT_SLAVE, 0);
		return -EIO;
	}
...
#endif
	default:
		ACT_LOG_ID_DBG(ALF_STR_bmi160_acc_config__ACCEL_ATTRIBUTE_NOT_, 0);
		return -ENOTSUP;
	}
			     ...
			     const struct sensor_value *val)
{
	...
	default:
		ACT_LOG_ID_DBG(ALF_STR_bmi160_gyr_config__GYRO_ATTRIBUTE_NOT_S, 0);
		return -ENOTSUP;
	}
...
#endif
	default:
		ACT_LOG_ID_DBG(ALF_STR_bmi160_attr_set__ATTR_SET_NOT_SUPPORT, 0);
		return -ENOTSUP;
	}
			      ...
			      struct sensor_value *val)
{
	...
	default:
		ACT_LOG_ID_DBG(ALF_STR_bmi160_channel_get__CHANNEL_NOT_SUPPORTE, 0);
		return -ENOTSUP;
	}
...
int bmi160_init(struct device *dev)
{
	...
	if (bmi160_byte_write(dev, BMI160_REG_CMD, BMI160_CMD_SOFT_RESET) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__CANNOT_REBOOT_CHIP, 0);
		return -EIO;
	}
	...
	if (bmi160_byte_read(dev, 0x7F, &val) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__CANNOT_READ_FROM_0X7, 0);
		return -EIO;
	}
	...
	if (bmi160_byte_read(dev, BMI160_REG_CHIPID, &val) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (val != BMI160_CHIP_ID) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__UNSUPPORTED_CHIP_DET, 1, val);
		return -ENODEV;
	}
	...
	if (bmi160_pmu_set(dev, &bmi160->pmu_sts) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__FAILED_TO_SET_POWER_, 0);
		return -EIO;
	}
	...
	if (bmi160_byte_write(dev, BMI160_REG_ACC_RANGE,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__CANNOT_SET_DEFAULT_R, 0);
		return -EIO;
	}
	...
	if (bmi160_reg_field_update(dev, BMI160_REG_ACC_CONF,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__FAILED_TO_SET_ACCELS, 0);
		return -EIO;
	}
	...
	if (bmi160_reg_field_update(dev, BMI160_REG_GYR_CONF,
		...
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__FAILED_TO_SET_GYROS_, 0);
		return -EIO;
	}

#ifdef CONFIG_BMI160_TRIGGER
	if (bmi160_trigger_mode_init(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_bmi160_init__CANNOT_SET_UP_TRIGGE, 0);
		return -EINVAL;
	}
#endif
