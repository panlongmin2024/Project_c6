...
#define ACT_LOG_MODEL_ID ALF_MODEL_LSM6DS0
...
static int lsm6ds0_sample_fetch_accel(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_sample_fetch_accel__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6ds0_sample_fetch_gyro(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_sample_fetch_gyro__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6ds0_sample_fetch_temp(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_sample_fetch_temp__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6ds0_init_chip(struct device *dev)
{
	...
	if (lsm6ds0_reboot(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__FAILED_TO_REBOOT_DEV, 0);
		return -EIO;
	}

	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
			      LSM6DS0_REG_WHO_AM_I, &chip_id) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__FAILED_READING_CHIP_, 0);
		return -EIO;
	}
	if (chip_id != LSM6DS0_VAL_WHO_AM_I) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		return -EIO;
	}
	ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__CHIP_ID_0XX, 1, chip_id);

	if (lsm6ds0_accel_axis_ctrl(dev, LSM6DS0_ACCEL_ENABLE_X_AXIS,
				    LSM6DS0_ACCEL_ENABLE_Y_AXIS,
				    LSM6DS0_ACCEL_ENABLE_Z_AXIS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__FAILED_TO_SET_ACCELE, 0);
		return -EIO;
	}
	...
	if (lsm6ds0_gyro_axis_ctrl(dev, LSM6DS0_GYRO_ENABLE_X_AXIS,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__FAILED_TO_SET_GYROSC, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
				...
				< 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init_chip__FAILED_TO_SET_BDU_BL, 0);
		return -EIO;
	}
...
static int lsm6ds0_init(struct device *dev)
{
	...
	if (lsm6ds0_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6ds0_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
