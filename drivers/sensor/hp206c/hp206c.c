...
#define ACT_LOG_MODEL_ID ALF_MODEL_HP206C
...
static int hp206c_init(struct device *dev)
{
	...
	if (!hp206c->i2c) {
		ACT_LOG_ID_ERR(ALF_STR_hp206c_init__I2C_MASTER_CONTROLLE, 0);
		return -EINVAL;
	}

	/* reset the chip */
	if (hp206c_cmd_send(dev, HP206C_CMD_SOFT_RST) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hp206c_init__CANNOT_RESET_CHIP, 0);
		return -EIO;
	}
	...
	if (hp206c_osr_set(dev, HP206C_DEFAULT_OSR) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hp206c_init__OSR_VALUE_IS_NOT_SUP, 0);
		return -ENOTSUP;
	}
