...
#define ACT_LOG_MODEL_ID ALF_MODEL_LIS3DH
...
int lis3dh_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, LIS3DH_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_lis3dh_sample_fetch__COULD_NOT_READ_ACCEL, 0);
		return -EIO;
	}
...
int lis3dh_init(struct device *dev)
{
	...
	if (i2c_reg_write_byte(drv_data->i2c, LIS3DH_I2C_ADDRESS,
		...
		ACT_LOG_ID_DBG(ALF_STR_lis3dh_init__FAILED_TO_CONFIGURE_, 0);
	}

	/* set full scale range */
	if (i2c_reg_write_byte(drv_data->i2c, LIS3DH_I2C_ADDRESS,
			       LIS3DH_REG_CTRL4, LIS3DH_FS_BITS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3dh_init__FAILED_TO_SET_FULL_S, 0);
		return -EIO;
	}

#ifdef CONFIG_LIS3DH_TRIGGER
	if (lis3dh_init_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3dh_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
