...
#define ACT_LOG_MODEL_ID ALF_MODEL_SHT3XD_TRIGGER
		    ...
		    const struct sensor_value *val)
{
	...
	if (sht3xd_write_reg(drv_data, set_cmd, reg_val) < 0 ||
		...
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_attr_set__FAILED_TO_WRITE_THRE, 0);
		return -EIO;
	}
...
int sht3xd_init_interrupt(struct device *dev)
{
	...
	if (sht3xd_write_reg(drv_data, SHT3XD_CMD_WRITE_TH_HIGH_SET, 0xFFFF)
			     < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_init_interrupt__FAILED_TO_WRITE_THRE, 0);
		return -EIO;
	}
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_init_interrupt__FAILED_TO_SET_GPIO_C, 0);
		return -EIO;
	}
