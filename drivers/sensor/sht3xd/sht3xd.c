...
#define ACT_LOG_MODEL_ID ALF_MODEL_SHT3XD
...
static int sht3xd_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_transfer(drv_data->i2c, msgs, 2, SHT3XD_I2C_ADDRESS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_sample_fetch__FAILED_TO_READ_DATA_, 0);
		return -EIO;
	}

	t_sample = (rx_buf[0] << 8) | rx_buf[1];
	if (sht3xd_compute_crc(t_sample) != rx_buf[2]) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_sample_fetch__RECEIVED_INVALID_TEM, 0);
		return -EIO;
	}

	rh_sample = (rx_buf[3] << 8) | rx_buf[4];
	if (sht3xd_compute_crc(rh_sample) != rx_buf[5]) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_sample_fetch__RECEIVED_INVALID_REL, 0);
		return -EIO;
	}
...
static int sht3xd_init(struct device *dev)
{
	...
	if (sht3xd_write_command(drv_data, SHT3XD_CMD_CLEAR_STATUS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_init__FAILED_TO_CLEAR_STAT, 0);
		return -EIO;
	}
	...
	if (sht3xd_write_command(drv_data,
		...
		< 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_init__FAILED_TO_SET_MEASUR, 0);
		return -EIO;
	}
	...
	if (sht3xd_init_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sht3xd_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
