...
#define ACT_LOG_MODEL_ID ALF_MODEL_HDC1008
...
static int hdc1008_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_write(drv_data->i2c, buf, 1, HDC1008_I2C_ADDRESS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_hdc1008_sample_fetch__FAILED_TO_WRITE_ADDR, 0);
		return -EIO;
	}
	...
	if (i2c_read(drv_data->i2c, buf, 4, HDC1008_I2C_ADDRESS) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_hdc1008_sample_fetch__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static u16_t read16(struct device *dev, u8_t a, u8_t d)
{
	...
	if (i2c_burst_read(dev, a, d, (u8_t *)buf, 2) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_read16__ERROR_READING_REGIST, 0);
	}
...
static int hdc1008_init(struct device *dev)
{
	...
	if (read16(drv_data->i2c, HDC1008_I2C_ADDRESS, HDC1000_MANUFID) != 0x5449) {
		ACT_LOG_ID_ERR(ALF_STR_hdc1008_init__FAILED_TO_GET_CORREC, 0);
		return -EINVAL;
	}
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_hdc1008_init__FAILED_TO_SET_GPIO_C, 0);
		return -EIO;
	}
