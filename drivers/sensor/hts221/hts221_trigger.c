...
#define ACT_LOG_MODEL_ID ALF_MODEL_HTS221_TRIGGER
...
int hts221_init_interrupt(struct device *dev)
{
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init_interrupt__COULD_NOT_SET_GPIO_C, 0);
		return -EIO;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, HTS221_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hts221_init_interrupt__COULD_NOT_ENABLE_DAT, 0);
		return -EIO;
	}
