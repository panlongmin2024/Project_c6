...
#define ACT_LOG_MODEL_ID ALF_MODEL_HTS221
...
static int hts221_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, HTS221_I2C_ADDR,
			   ...
			   buf, 4) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_sample_fetch__FAILED_TO_FETCH_DATA, 0);
		return -EIO;
	}
...
static int hts221_read_conversion_data(struct hts221_data *drv_data)
{
	...
	if (i2c_burst_read(drv_data->i2c, HTS221_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hts221_read_conversion_data__FAILED_TO_READ_CONVE, 0);
		return -EIO;
	}
...
int hts221_init(struct device *dev)
{
	...
	if (i2c_reg_read_byte(drv_data->i2c, HTS221_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (id != HTS221_CHIP_ID) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__INVALID_CHIP_ID, 0);
		return -EINVAL;
	}
	...
	if (idx == ARRAY_SIZE(hts221_odr_strings)) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__INVALID_ODR_VALUE, 0);
		return -EINVAL;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, HTS221_I2C_ADDR, HTS221_REG_CTRL1,
			       ...
			       HTS221_PD_BIT) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__FAILED_TO_CONFIGURE_, 0);
		return -EIO;
	}

	if (hts221_read_conversion_data(drv_data) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__FAILED_TO_READ_CONVE, 0);
		return -EINVAL;
	}

#ifdef CONFIG_HTS221_TRIGGER
	if (hts221_init_interrupt(dev) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_hts221_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
