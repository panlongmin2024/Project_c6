...
#define ACT_LOG_MODEL_ID ALF_MODEL_SX9500
...
int sx9500_init(struct device *dev)
{
	...
	if (sx9500_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sx9500_init__SX9500_FAILED_TO_INI, 0);
		return -EINVAL;
	}

	if (sx9500_setup_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sx9500_init__SX9500_FAILED_TO_SET, 0);
		return -EINVAL;
	}
