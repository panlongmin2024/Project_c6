...
#define ACT_LOG_MODEL_ID ALF_MODEL_SX9500_TRIGGER
...
static void sx9500_thread_main(int arg1, int unused)
{
	...
	while (1) {
		...
		if (i2c_reg_read_byte(data->i2c_master, data->i2c_slave_addr,
			...
			ACT_LOG_ID_DBG(ALF_STR_sx9500_thread_main__SX9500_ERROR_READING, 0);
			continue;
		}
...
static void sx9500_gpio_thread_cb(void *arg)
{
	...
	if (i2c_reg_read_byte(data->i2c_master, data->i2c_slave_addr,
			      SX9500_REG_IRQ_SRC, &reg_val) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_sx9500_gpio_thread_cb__SX9500_ERROR_READING, 0);
		return;
	}
