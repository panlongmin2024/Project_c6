...
#define ACT_LOG_MODEL_ID ALF_MODEL_LSM6DSL
...
static int lsm6dsl_sample_fetch_accel(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_sample_fetch_accel__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6dsl_sample_fetch_gyro(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_sample_fetch_gyro__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6dsl_sample_fetch_temp(struct device *dev)
{
	...
	if (i2c_burst_read(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_sample_fetch_temp__FAILED_TO_READ_SAMPL, 0);
		return -EIO;
	}
...
static int lsm6dsl_init_chip(struct device *dev)
{
	...
	if (lsm6dsl_reboot(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_TO_REBOOT_DEV, 0);
		return -EIO;
	}

	if (i2c_reg_read_byte(data->i2c_master, config->i2c_slave_addr,
			      LSM6DSL_REG_WHO_AM_I, &chip_id) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_READING_CHIP_, 0);
		return -EIO;
	}
	if (chip_id != LSM6DSL_VAL_WHO_AM_I) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__INVALID_CHIP_ID_0XX, 1, chip_id);
		return -EIO;
	}

	ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__CHIP_ID_0XX, 1, chip_id);

	if (lsm6dsl_accel_set_fs_raw(dev, LSM6DSL_DEFAULT_ACCEL_FULLSCALE)
				     < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_TO_SET_ACCELE, 0);
		return -EIO;
	}
	...
	if (lsm6dsl_gyro_set_fs_raw(dev, LSM6DSL_DEFAULT_GYRO_FULLSCALE)
				    < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_TO_SET_GYROSC, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
		...
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_TO_SET_FIFO_M, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c_master, config->i2c_slave_addr,
				...
				< 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init_chip__FAILED_TO_SET_BDU_BL, 0);
		return -EIO;
	}
...
static int lsm6dsl_init(struct device *dev)
{
	...
	if (lsm6dsl_init_chip(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lsm6dsl_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
