...
#define ACT_LOG_MODEL_ID ALF_MODEL_FXOS8700_TRIGGER
...
static int fxos8700_handle_pulse_int(struct device *dev)
{
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
			      ...
			      &pulse_source)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_handle_pulse_int__COULD_NOT_READ_PULSE, 0);
	}
...
static void fxos8700_handle_int(void *arg)
{
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
			      ...
			      &int_source)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_handle_int__COULD_NOT_READ_INTER, 0);
		int_source = 0;
	}
			 ...
			 sensor_trigger_handler_t handler)
{
...
#endif
	default:
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_set__UNSUPPORTED_SENSOR_T, 0);
		ret = -ENOTSUP;
		goto exit;
	}
	...
	if (fxos8700_get_power(dev, &power)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_set__COULD_NOT_GET_POWER_, 0);
		ret = -EIO;
		goto exit;
	}
	...
	if (fxos8700_set_power(dev, FXOS8700_POWER_STANDBY)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_set__COULD_NOT_SET_STANDB, 0);
		ret = -EIO;
		goto exit;
	}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
				...
				mask,
				handler ? mask : 0)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_set__COULD_NOT_CONFIGURE_, 0);
		ret = -EIO;
		goto exit;
	}
	...
	if (fxos8700_set_power(dev, power)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_set__COULD_NOT_RESTORE_PO, 0);
		ret = -EIO;
		goto exit;
	}
...
int fxos8700_trigger_init(struct device *dev)
{
	...
	if (i2c_reg_write_byte(data->i2c, config->i2c_address,
		...
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_init__COULD_NOT_CONFIGURE_, 0);
		return -EIO;
	}
	...
	if (data->gpio == NULL) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_trigger_init__COULD_NOT_FIND_GPIO_, 0);
		return -EINVAL;
	}
