...
#define ACT_LOG_MODEL_ID ALF_MODEL_FXOS8700
...
static int fxos8700_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (chan != SENSOR_CHAN_ALL) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_sample_fetch__UNSUPPORTED_SENSOR_C, 0);
		return -ENOTSUP;
	}
	...
	if (i2c_burst_read(data->i2c, config->i2c_address, config->start_addr,
			   buffer, num_bytes)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_sample_fetch__COULD_NOT_FETCH_SAMP, 0);
		ret = -EIO;
		goto exit;
	}
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address, FXOS8700_REG_TEMP,
			      &data->temp)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_sample_fetch__COULD_NOT_FETCH_TEMP, 0);
		ret = -EIO;
		goto exit;
	}
#endif
				...
				struct sensor_value *val)
{
	...
	if (ret != 0) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_channel_get__UNSUPPORTED_SENSOR_C, 0);
	}
...
int fxos8700_get_power(struct device *dev, enum fxos8700_power *power)
{
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
			      ...
			      &val)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_get_power__COULD_NOT_GET_POWER_, 0);
		return -EIO;
	}
...
static int fxos8700_init(struct device *dev)
{
	...
	if (data->i2c == NULL) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_FIND_I2C_D, 0);
		return -EINVAL;
	}
	...
	*/
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
		...
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_GET_WHOAMI, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
				...
				config->mode)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_SET_MODE, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
				...
				FXOS8700_M_CTRLREG2_AUTOINC_MASK)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_SET_HYBRID, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
				...
				config->range)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_SET_RANGE, 0);
		return -EIO;
	}

#if CONFIG_FXOS8700_TRIGGER
	if (fxos8700_trigger_init(dev)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_INITIALIZE, 0);
		return -EIO;
	}
#endif
	...
	if (fxos8700_set_power(dev, FXOS8700_POWER_ACTIVE)) {
		ACT_LOG_ID_ERR(ALF_STR_fxos8700_init__COULD_NOT_SET_ACTIVE, 0);
		return -EIO;
	}

	k_sem_init(&data->sem, 1, UINT_MAX);

	ACT_LOG_ID_DBG(ALF_STR_fxos8700_init__INIT_COMPLETE, 0);
