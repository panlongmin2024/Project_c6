...
#define ACT_LOG_MODEL_ID ALF_MODEL_LIS3MDL
...
int lis3mdl_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, CONFIG_LIS3MDL_I2C_ADDR,
		...
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_sample_fetch__FAILED_TO_FETCH_MEGN, 0);
		return -EIO;
	}
	...
	if (i2c_burst_read(drv_data->i2c, CONFIG_LIS3MDL_I2C_ADDR,
			   ...
			   (u8_t *)(buf + 3), 2) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_sample_fetch__FAILED_TO_FETCH_TEMP, 0);
		return -EIO;
	};
...
int lis3mdl_init(struct device *dev)
{
	...
	if (i2c_reg_read_byte(drv_data->i2c, CONFIG_LIS3MDL_I2C_ADDR,
		...
		ACT_LOG_ID_ERR(ALF_STR_lis3mdl_init__FAILED_TO_READ_CHIP_, 0);
		return -EIO;
	}

	if (id != LIS3MDL_CHIP_ID) {
		ACT_LOG_ID_ERR(ALF_STR_lis3mdl_init__INVALID_CHIP_ID, 0);
		return -EINVAL;
	}
	...
	if (idx == ARRAY_SIZE(lis3mdl_odr_strings)) {
		ACT_LOG_ID_ERR(ALF_STR_lis3mdl_init__INVALID_ODR_VALUE, 0);
		return -EINVAL;
	}
	...
	if (i2c_write(drv_data->i2c,
		...
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_init__FAILED_TO_CONFIGURE_, 0);
		return -EIO;
	}

#ifdef CONFIG_LIS3MDL_TRIGGER
	if (lis3mdl_init_interrupt(dev) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_init__FAILED_TO_INITIALIZE, 0);
		return -EIO;
	}
#endif
