...
#define ACT_LOG_MODEL_ID ALF_MODEL_LIS3MDL_TRIGGER
...
int lis3mdl_init_interrupt(struct device *dev)
{
	...
	if (gpio_add_callback(drv_data->gpio, &drv_data->gpio_cb) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_init_interrupt__COULD_NOT_SET_GPIO_C, 0);
		return -EIO;
	}

	/* clear data ready interrupt line by reading sample data */
	if (lis3mdl_sample_fetch(dev, SENSOR_CHAN_ALL) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_init_interrupt__COULD_NOT_CLEAR_DATA, 0);
		return -EIO;
	}
	...
	if (i2c_reg_write_byte(drv_data->i2c, CONFIG_LIS3MDL_I2C_ADDR,
		...
		ACT_LOG_ID_DBG(ALF_STR_lis3mdl_init_interrupt__COULD_NOT_ENABLE_INT, 0);
		return -EIO;
	}
