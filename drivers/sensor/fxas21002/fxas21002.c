...
#define ACT_LOG_MODEL_ID ALF_MODEL_FXAS21002
...
static int fxas21002_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (chan != SENSOR_CHAN_ALL) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_sample_fetch__UNSUPPORTED_SENSOR_C, 0);
		return -ENOTSUP;
	}
	...
	if (i2c_burst_read(data->i2c, config->i2c_address,
		...
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_sample_fetch__COULD_NOT_FETCH_SAMP, 0);
		ret = -EIO;
		goto exit;
	}
				 ...
				 struct sensor_value *val)
{
	...
	if (ret != 0) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_channel_get__UNSUPPORTED_SENSOR_C, 0);
	}
...
int fxas21002_get_power(struct device *dev, enum fxas21002_power *power)
{
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
			      ...
			      &val)) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_get_power__COULD_NOT_GET_POWER_, 0);
		return -EIO;
	}
...
static int fxas21002_init(struct device *dev)
{
	...
	if (data->i2c == NULL) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_FIND_I2C_D, 0);
		return -EINVAL;
	}
	...
	if (i2c_reg_read_byte(data->i2c, config->i2c_address,
		...
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_GET_WHOAMI, 0);
		return -EIO;
	}
	...
	do {
		if (i2c_reg_read_byte(data->i2c, config->i2c_address,
			...
			ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_GET_CTRLRE, 0);
			return -EIO;
		}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
				...
				config->range)) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_SET_RANGE, 0);
		return -EIO;
	}
	...
	if (i2c_reg_update_byte(data->i2c, config->i2c_address,
		...
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_SET_OUTPUT, 0);
		return -EIO;
	}

#if CONFIG_FXAS21002_TRIGGER
	if (fxas21002_trigger_init(dev)) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_INITIALIZE, 0);
		return -EIO;
	}
#endif
	...
	if (fxas21002_set_power(dev, FXAS21002_POWER_ACTIVE)) {
		ACT_LOG_ID_ERR(ALF_STR_fxas21002_init__COULD_NOT_SET_ACTIVE, 0);
		return -EIO;
	}
	...
	ACT_LOG_ID_DBG(ALF_STR_fxas21002_init__INIT_COMPLETE, 0);
