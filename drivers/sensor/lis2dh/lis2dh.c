...
#define ACT_LOG_MODEL_ID ALF_MODEL_LIS2DH
...
static int lis2dh_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (status < 0) {
		ACT_LOG_ID_WRN(ALF_STR_lis2dh_sample_fetch__COULD_NOT_READ_ACCEL, 0);
		return status;
	}
...
#endif
	default:
		ACT_LOG_ID_DBG(ALF_STR_lis2dh_acc_config__ACCEL_ATTRIBUTE_NOT_, 0);
		return -ENOTSUP;
	}
			   ...
			   const struct sensor_value *val)
{
	...
	default:
		ACT_LOG_ID_WRN(ALF_STR_lis2dh_attr_set__ATTR_SET_NOT_SUPPORT, 0);
		return -ENOTSUP;
	}
...
int lis2dh_init(struct device *dev)
{
	...
	if (status < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lis2dh_init__FAILED_TO_CONFIGURE_, 0);
		return status;
	}
	...
	status = lis2dh_burst_write(lis2dh->bus, LIS2DH_REG_CTRL1, raw,
				    sizeof(raw));
	if (status < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lis2dh_init__FAILED_TO_RESET_CTRL, 0);
		return status;
	}
	...
	status = lis2dh_reg_write_byte(lis2dh->bus, LIS2DH_REG_CTRL4,
				       LIS2DH_FS_BITS);
	if (status < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lis2dh_init__FAILED_TO_SET_FULL_S, 0);
		return status;
	}
	...
	if (status < 0) {
		ACT_LOG_ID_ERR(ALF_STR_lis2dh_init__FAILED_TO_INITIALIZE, 0);
		return status;
	}
#endif
