...
#define ACT_LOG_MODEL_ID ALF_MODEL_ETH_SAM_GMAC
...
static int rx_descriptors_init(Gmac *gmac, struct gmac_queue *queue)
{
	...
	for (int i = 0; i < rx_desc_list->len; i++) {
		...
		if (rx_buf == NULL) {
			...
			ACT_LOG_ID_ERR(ALF_STR_rx_descriptors_init__FAILED_TO_RESERVE_DA, 0);
			return -ENOBUFS;
		}
...
static void tx_completed(Gmac *gmac, struct gmac_queue *queue)
{
	...
	while (tx_desc_list->tail != tx_desc_list->head) {
		...
		if (tx_desc->w1 & GMAC_TXW1_LASTBUFFER) {
			...
			net_pkt_unref(pkt);
			ACT_LOG_ID_DBG(ALF_STR_tx_completed__DROPPING_PKT_08X, 1, pkt);
...
static void tx_error_handler(Gmac *gmac, struct gmac_queue *queue)
{
	...
	while (tx_frames->tail != tx_frames->head) {
		...
		net_pkt_unref(pkt);
		ACT_LOG_ID_DBG(ALF_STR_tx_error_handler__DROPPING_PKT_08X, 1, pkt);
...
static int get_mck_clock_divisor(u32_t mck)
{
	...
	if (mck <= 20000000) {
		...
		ACT_LOG_ID_ERR(ALF_STR_get_mck_clock_divisor__NO_VALID_MDC_CLOCK, 0);
		mck_divisor = -ENOTSUP;
	}
...
static int queue_init(Gmac *gmac, struct gmac_queue *queue)
{
	...
	ACT_LOG_ID_INF(ALF_STR_queue_init__QUEUE_D_ACTIVATED, 1, queue->que_idx);
...
static struct net_pkt *frame_get(struct gmac_queue *queue)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_frame_get__FRAME_COMPLETE_RX08X, 2, rx_frame, tail);
...
static void eth_rx(struct gmac_queue *queue)
{
	...
	while (rx_frame) {
		ACT_LOG_ID_DBG(ALF_STR_eth_rx__ETH_RX, 0);
...
static int eth_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_eth_tx__ETH_TX, 0);
...
static void queue0_isr(void *arg)
{
	...
	isr = gmac->GMAC_ISR;
	ACT_LOG_ID_DBG(ALF_STR_queue0_isr__GMAC_ISR0X08X, 1, isr);
	...
	if (isr & GMAC_IER_HRESP) {
		ACT_LOG_ID_DBG(ALF_STR_queue0_isr__HRESP, 0);
	}
}
...
void get_mac_addr_from_i2c_eeprom(u8_t mac_addr[6])
{
	...
	if (!dev) {
		ACT_LOG_ID_ERR(ALF_STR_get_mac_addr_from_i2c_eeprom__I2C_DEVICE_NOT_FOUND, 0);
		return;
	}
...
static void eth0_iface_init(struct net_if *iface)
{
	...
	if (result < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth0_iface_init__UNABLE_TO_INITIALIZE, 0);
		return;
	}
	...
	if (result < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth0_iface_init__ETH_PHY_INITIALIZATI, 0);
		return;
	}
	/* PHY auto-negotiate link parameters */
	result = phy_sam_gmac_auto_negotiate(&cfg->phy, &link_status);
	if (result < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth0_iface_init__ETH_PHY_AUTONEGOTIAT, 0);
		return;
	}
