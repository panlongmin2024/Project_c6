...
#define ACT_LOG_MODEL_ID ALF_MODEL_PHY_SAM_GMAC
...
static int mdio_bus_wait(Gmac *gmac)
{
	...
	while (!(gmac->GMAC_NSR & GMAC_NSR_IDLE))   {
		if (retries-- == 0) {
			ACT_LOG_ID_ERR(ALF_STR_mdio_bus_wait__TIMEOUT, 0);
			return -ETIMEDOUT;
		}
...
int phy_sam_gmac_init(const struct phy_sam_gmac_dev *phy)
{
	...
	ACT_LOG_ID_INF(ALF_STR_phy_sam_gmac_init__SOFT_RESET_OF_ETH_PH, 0);
	phy_soft_reset(phy);

	/* Verify that the PHY device is responding */
	phy_id = phy_sam_gmac_id_get(phy);
	if (phy_id == 0xFFFFFFFF) {
		ACT_LOG_ID_ERR(ALF_STR_phy_sam_gmac_init__UNABLE_TO_DETECT_A_V, 0);
		return -1;
	}

	ACT_LOG_ID_INF(ALF_STR_phy_sam_gmac_init__PHYID_0XX_AT_ADDR_D, 2, phy_id, phy->address);
				...
				u32_t *status)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_phy_sam_gmac_auto_negotiate__STARTING_ETH_PHY_AUT, 0);
	...
	ACT_LOG_ID_DBG(ALF_STR_phy_sam_gmac_auto_negotiate__PHY_AUTONEGOTIATE_SE, 0);
