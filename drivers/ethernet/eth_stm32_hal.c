...
#define ACT_LOG_MODEL_ID ALF_MODEL_ETH_STM32_HAL
...
static int eth_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (total_len > ETH_TX_BUF_SIZE) {
		ACT_LOG_ID_ERR(ALF_STR_eth_tx__PKT_TO_BIGN, 0);
		res = -EIO;
		goto error;
	}
	...
	if (HAL_ETH_TransmitFrame(heth, total_len) != HAL_OK) {
		ACT_LOG_ID_ERR(ALF_STR_eth_tx__HAL_ETH_TRANSMITFRAM, 0);
		res = -EIO;
		goto error;
	}
...
static void eth0_iface_init(struct net_if *iface)
{
	...
	if (HAL_ETH_Init(heth) != HAL_OK) {
		ACT_LOG_ID_ERR(ALF_STR_eth0_iface_init__HAL_ETH_INIT_FAILEDN, 0);
	}
