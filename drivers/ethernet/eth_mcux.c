...
#define ACT_LOG_MODEL_ID ALF_MODEL_ETH_MCUX
...
static void eth_mcux_phy_event(struct eth_context *context)
{
	...
	case eth_mcux_phy_state_read_status:
		...
		if (link_up && !context->link_up) {
			...
			ACT_LOG_ID_INF(ALF_STR_eth_mcux_phy_event__LINK_DOWN, 0);
			context->link_up = link_up;
...
static int eth_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (status) {
		ACT_LOG_ID_ERR(ALF_STR_eth_tx__ENET_SENDFRAME_ERROR, 1, status);
		return -1;
	}
...
static void eth_rx(struct device *iface)
{
	...
	if (status) {
		...
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__ENET_GETRXFRAMESIZE_, 1, status);
	...
	if (sizeof(context->frame_buf) < frame_length) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FRAME_TOO_LARGE_DN, 1, frame_length);
		net_pkt_unref(pkt);
		status = ENET_ReadFrame(ENET, &context->enet_handle, NULL, 0);
		assert(status == kStatus_Success);
		return;
	...
	if (status) {
		irq_unlock(imask);
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__ENET_READFRAME_FAILE, 1, status);
		net_pkt_unref(pkt);
		return;
	}
	...
	do {
		...
		if (!pkt_buf) {
			irq_unlock(imask);
			ACT_LOG_ID_ERR(ALF_STR_eth_rx__FAILED_TO_GET_FRAGME, 0);
			net_pkt_unref(pkt);
			assert(status == kStatus_Success);
			return;
		}
