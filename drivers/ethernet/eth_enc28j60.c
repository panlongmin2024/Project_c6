...
#define ACT_LOG_MODEL_ID ALF_MODEL_ETH_ENC28J60
...
static int eth_enc28j60_init(struct device *dev)
{
	...
	if (spi_configure(context->spi, &spi_cfg) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth_enc28j60_init__FAILED_TO_CONFIGURE_, 0);
		return -EIO;
	}
	...
	if (eth_enc28j60_soft_reset(dev)) {
		ACT_LOG_ID_ERR(ALF_STR_eth_enc28j60_init__SOFTRESET_FAILED, 0);
		return -EIO;
	}
	...
	ACT_LOG_ID_INF(ALF_STR_eth_enc28j60_init__ENC28J60_INITIALIZED, 0);
			   ...
			   u16_t len)
{
	...
	*/
	...
	if (tx_end & ENC28J60_BIT_ESTAT_TXABRT) {
		ACT_LOG_ID_ERR(ALF_STR_eth_enc28j60_tx__TX_FAILED, 0);
		return -EIO;
	}
...
static int eth_enc28j60_rx(struct device *dev)
{
	...
	*/
	...
	ACT_LOG_ID_DBG(ALF_STR_eth_enc28j60_rx__, 0);
	...
	do {
		...
		if (!pkt) {
			ACT_LOG_ID_ERR(ALF_STR_eth_enc28j60_rx__COULD_NOT_ALLOCATE_R, 0);
			goto done;
		}
		...
		do {
			...
			if (!pkt_buf) {
				ACT_LOG_ID_ERR(ALF_STR_eth_enc28j60_rx__COULD_NOT_ALLOCATE_D, 0);
				net_pkt_unref(pkt);
		...
		ACT_LOG_ID_DBG(ALF_STR_eth_enc28j60_rx__RECEIVED_PACKET_OF_L, 1, lengthfr);
...
static int eth_net_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_eth_net_tx__PKT_08X_LEN_U, 2, pkt, len);
...
static void eth_enc28j60_iface_init_0(struct net_if *iface)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_eth_enc28j60_iface_init_0__, 0);
