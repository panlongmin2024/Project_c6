...
#define ACT_LOG_MODEL_ID ALF_MODEL_ETH_DW
...
static void eth_rx(struct device *port)
{
	...
	if (context->rx_desc.own) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__SPURIOUS_RECEIVE_INT, 0);
		return;
	}
	...
	if (frm_len > sizeof(context->rx_buf)) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FRAME_TOO_LARGE_U, 1, frm_len);
		goto release_desc;
	}
	...
	if (frm_len < sizeof(u32_t)) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FRAME_TOO_SMALL_U, 1, frm_len);
		goto release_desc;
	} else {
	...
	if (!pkt) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FAILED_TO_OBTAIN_RX_, 0);
		goto release_desc;
	}

	if (!net_pkt_append_all(pkt, frm_len, (u8_t *)context->rx_buf,
				K_NO_WAIT)) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FAILED_TO_APPEND_RX_, 0);
		net_pkt_unref(pkt);
		goto release_desc;
	}
	...
	if (r < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth_rx__FAILED_TO_ENQUEUE_FR, 1, r);
		net_pkt_unref(pkt);
	}
...
static int eth_initialize_internal(struct net_if *iface)
{
	...
	ACT_LOG_ID_INF(ALF_STR_eth_initialize_internal__ENABLED_100M_FULLDUP, 0);
...
static void eth_initialize(struct net_if *iface)
{
	...
	if (r < 0) {
		ACT_LOG_ID_ERR(ALF_STR_eth_initialize__COULD_NOT_INITIALIZE, 1, r);
	}
}
