...
#define ACT_LOG_MODEL_ID ALF_MODEL_H5
...
static void hexdump(const char *str, const u8_t *packet, size_t length)
{
	...
	while (length--) {
		...
		ACT_LOG_ID_INF(ALF_STR_hexdump__02X_, 1, *packet++);

		n++;
		if (n % 8 == 0) {
			if (n % 16 == 0) {
				ACT_LOG_ID_INF(ALF_STR_hexdump__N, 0);
			} else {
				ACT_LOG_ID_INF(ALF_STR_hexdump___, 0);
			}
		}
	}
