...
#define ACT_LOG_MODEL_ID ALF_MODEL_QUARK_SE_CLOCK_CONTROL
					    ...
					    clock_control_subsys_t sub_system)
{
	...
	if (sub_system == CLOCK_CONTROL_SUBSYS_ALL) {
		ACT_LOG_ID_DBG(ALF_STR_quark_se_clock_control_on__ENABLING_ALL_CLOCK_G, 1, dev);
		sys_write32(0xffffffff, info->base_address);

		return 0;
	}

	ACT_LOG_ID_DBG(ALF_STR_quark_se_clock_control_on__ENABLING_CLOCK_GATE_, 2, dev, subsys);
					     ...
					     clock_control_subsys_t sub_system)
{
	...
	if (sub_system == CLOCK_CONTROL_SUBSYS_ALL) {
		ACT_LOG_ID_DBG(ALF_STR_quark_se_clock_control_off__DISABLING_ALL_CLOCK_, 1, dev);
		sys_write32(0x00000000, info->base_address);

		return 0;
	}

	ACT_LOG_ID_DBG(ALF_STR_quark_se_clock_control_off__CLOCK_GATE_ON_DEV_08, 2, dev, subsys);
