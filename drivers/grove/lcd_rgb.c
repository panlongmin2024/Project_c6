...
#define ACT_LOG_MODEL_ID ALF_MODEL_LCD_RGB
...
void glcd_clear(struct device *port)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_glcd_clear__CLEAR_DELAY_20_MS, 0);
	_sleep(20);
}
...
void glcd_display_state_set(struct device *port, u8_t opt)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_glcd_display_state_set__SET_DISPLAY_STATE_OP, 0);
	_sleep(5);
}
...
void glcd_input_state_set(struct device *port, u8_t opt)
{
	...
	i2c_write(dev->i2c, &dev->input_set, sizeof(dev->input_set),
		  rom->lcd_addr);
	ACT_LOG_ID_DBG(ALF_STR_glcd_input_state_set__SET_THE_INPUT_SET_NO, 0);
}
...
{
	if (color > 3) {
		ACT_LOG_ID_WRN(ALF_STR_glcd_color_select__SELECTED_COLOR_IS_TO, 0);
		return;
	}
...
void glcd_function_set(struct device *port, u8_t opt)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_glcd_function_set__SET_FUNCTION_OPTIONS, 0);
	_sleep(5);
}
...
int glcd_initialize(struct device *port)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_glcd_initialize__INITIALIZE_CALLED, 0);
	 ...
	 */
	ACT_LOG_ID_DBG(ALF_STR_glcd_initialize__DELAY_50_MS_WHILE_TH, 0);
	_sleep(50);
	...
	ACT_LOG_ID_INF(ALF_STR_glcd_initialize__CONFIGURING_THE_RGB_, 0);
	...
	ACT_LOG_ID_DBG(ALF_STR_glcd_initialize__BACKGROUND_SET_TO_WH, 0);
