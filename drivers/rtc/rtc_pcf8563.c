...
#define ACT_LOG_MODEL_ID ALF_MODEL_RTC_PCF8563
...
static void pcf8563_dump_regs(struct pcf8563_info *pcf8563)
{
	...
	ACT_LOG_ID_INF(ALF_STR_bin2bcd__PCF8563_REGISTERSN, 0);
	ret = i2c_burst_read(pcf8563->i2c, PCF8563_I2C_ADDR, PCF8563_REG_ST1, buf, 16);
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_bin2bcd__FAILED_TO_READ_REGIS, 1, ret);
		return;
	}

	for (i = 0; i < 16; i++) {
		ACT_LOG_ID_INF(ALF_STR_bin2bcd____02D_0X02XN, 2, i, buf[i]);
	}
}
#endif
...
static int pcf8563_get_datetime(struct pcf8563_info *pcf8563, struct rtc_time *tm)
{
	...
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_get_datetime__FAILED_TO_GET_DATETI, 0);
		return ret;
	}

	if (buf[0] & PCF8563_SC_LV) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_get_datetime__LOW_VOLTAGE_DETECTED, 0);
	}
	...
	if (rtc_valid_tm(tm) < 0)
		ACT_LOG_ID_INF(ALF_STR_pcf8563_get_datetime__PCF8563_RETRIEVED_DA, 0);
...
static int pcf8563_set_datetime(struct pcf8563_info *pcf8563, struct rtc_time *tm)
{
	...
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_set_datetime__FAILED_TO_SET_DATETI, 0);
		return ret;
	}
...
static int pcf8563_set_alarm_time(struct pcf8563_info *pcf8563, struct rtc_time *tm)
{
	...
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_set_alarm_time__FAILED_TO_SET_ALARM, 0);
		return ret;
	}
...
static int pcf8563_set_alarm(struct device *dev, const uint32_t alarm_val)
{
	...
	ACT_LOG_ID_INF(ALF_STR_pcf8563_set_alarm__SET_ALARM_D, 1, alarm_val);
...
static uint32_t pcf8563_read(struct device *dev)
{
	...
	if (ret) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_read__FAILED_TO_GET_DATETI, 0);
		return 0;
	}
...
static void pcf8563_check_status(struct pcf8563_info *pcf8563)
{
	...
	if (val != 0x08) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_check_status__RTC_IS_STOPED_ENABLE, 0);
...
static int pcf8563_check_datetime(struct pcf8563_info *pcf8563)
{
	...
	if (rtc_valid_tm(&tm) < 0) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_check_datetime__INVALID_DATE_SET_DEF, 0);

		rtc_time_to_tm(PCF8563_DEFAULT_INIT_TIME, &tm);
		pcf8563_set_datetime(pcf8563, &tm);
	}

	ACT_LOG_ID_INF(ALF_STR_pcf8563_check_datetime__CURRENT_TIME_, 0);
	print_rtc_time(&tm);
...
static int pcf8563_init(struct device *dev)
{
	...
	ACT_LOG_ID_INF(ALF_STR_pcf8563_init__INIT_PCF8563, 0);

	pcf8563->i2c = device_get_binding(CONFIG_RTC_PCF8563_I2C_NAME);
	if (!pcf8563->i2c) {
		ACT_LOG_ID_ERR(ALF_STR_pcf8563_init__DEVICE_NOT_FOUNDN, 0);
		return -1;
	}
