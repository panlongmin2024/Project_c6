...
#define ACT_LOG_MODEL_ID ALF_MODEL_SLIP
#include <stdio.h>
#endif
...
static u8_t *recv_cb(u8_t *buf, size_t *off)
{
	...
	for (i = 0; i < *off; i++) {
		if (slip_input_byte(slip, buf[i])) {
			...
			ACT_LOG_ID_DBG(ALF_STR_recv_cb__08X_RECEIVED_DATA_D_, 2, slip, bytes);
#endif
			process_msg(slip);
			break;
		}
...
static int slip_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_slip_init__08X_DEV_08X, 2, slip, dev);
	...
	ACT_LOG_ID_DBG(ALF_STR_slip_init__ARP_ENABLED, 0);
#endif
