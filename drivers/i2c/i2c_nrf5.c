...
#define ACT_LOG_MODEL_ID ALF_MODEL_I2C_NRF5
...
static int i2c_nrf5_configure(struct device *dev, u32_t dev_config_raw)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_configure__, 0);
	...
	case I2C_SPEED_FAST:
		...
		break;
	default:
		ACT_LOG_ID_ERR(ALF_STR_i2c_nrf5_configure__UNSUPPORTED_SPEED, 0);
		return -EINVAL;
	}
...
static int i2c_nrf5_read(struct device *dev, struct i2c_msg *msg)
{
	...
	for (int offset = 0; offset < msg->len; offset++) {
		if (offset == msg->len-1) {
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__SHORTS2, 0);
			twi->SHORTS = 2; /* BB->STOP */
		} else {
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__SHORTS1, 0);
			twi->SHORTS = 1; /* BB->SUSPEND */
		}

		if (offset == 0) {
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__STARTRX, 0);
			twi->TASKS_STARTRX = 1;
		} else {
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__RESUME, 0);
			twi->TASKS_RESUME = 1;
		}
		...
		if (data->err) {
			data->err = 0;
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__RX_ERROR_0XX, 1, twi->ERRORSRC);
			twi->TASKS_STOP = 1;
		...
		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__RXD, 0);
		data->rxd = 0;
		msg->buf[offset] = twi->RXD;
	}

	if (msg->flags & I2C_MSG_STOP) {
		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_read__TASK_STOP, 0);
		k_sem_take(&data->sem, K_FOREVER);
			  ...
			  struct i2c_msg *msg)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__, 0);
	...
	for (int offset = 0; offset < msg->len; offset++) {
		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__TXD0XX, 1, msg->buf[offset]);
		twi->TXD = msg->buf[offset];

		if (offset == 0) {
			ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__STARTTX, 0);
			twi->TASKS_STARTTX = 1;
		}

		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__WAIT_FOR_SYNC, 0);
		k_sem_take(&data->sem, K_FOREVER);
		...
		data->txd = 0;
		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__TXDSENT_ARRIVED, 0);
	}

	if (msg->flags & I2C_MSG_STOP) {
		ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_write__TASK_STOP, 0);
		twi->TASKS_STOP = 1;
		k_sem_take(&data->sem, K_FOREVER);
		SYS_LOG_DBG("err=%d txd=%d rxd=%d stopped=%d errsrc=0x%x",
			    data->err, data->txd, data->rxd,
			     ...
			     u8_t num_msgs, u16_t addr)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_transfer__TRANSACTIONSTART_ADD, 1, addr);
...
static int i2c_nrf5_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_i2c_nrf5_init__, 0);
