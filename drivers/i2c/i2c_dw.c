...
#define ACT_LOG_MODEL_ID ALF_MODEL_I2C_DW
...
static void i2c_dw_isr(void *arg)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_i2c_dw_isr__I2C_INTERRUPT_RECEIV, 0);
...
static int _i2c_dw_setup(struct device *dev, u16_t slave_address)
{
	...
	if (dw->app_config.bits.is_master_device) {
		 ...
		 * to both 0 or both 1
		 */
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_HOST_CONFIGURED_, 0);
	...
	if (dw->app_config.bits.use_10_bit_addr) {
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_USING_10BIT_ADDR, 0);
	...
	case I2C_SPEED_STANDARD:
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_SPEED_SET_TO_STA, 0);
	...
	case I2C_SPEED_FAST_PLUS:
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_SPEED_SET_TO_FAS, 0);
	...
	case I2C_SPEED_HIGH:
		...
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_SPEED_SET_TO_HIG, 0);
		...
		break;
	default:
		ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_INVALID_SPEED_RE, 0);
		return -EINVAL;
	}

	ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_LCNT__D, 1, dw->lcnt);
	ACT_LOG_ID_DBG(ALF_STR__i2c_dw_setup__I2C_HCNT__D, 1, dw->hcnt);
...
static inline int i2c_dw_pci_setup(struct device *dev)
{
	...
	if (!pci_bus_scan(&dw->pci_dev)) {
		ACT_LOG_ID_DBG(ALF_STR_i2c_dw_pci_setup__COULD_NOT_FIND_DEVIC, 0);
		return 0;
	}
...
static int i2c_dw_initialize(struct device *port)
{
	...
	if (regs->ic_con.bits.speed == I2C_DW_SPEED_HIGH) {
		ACT_LOG_ID_DBG(ALF_STR_i2c_dw_initialize__I2C_HIGH_SPEED_SUPPO, 0);
		dev->support_hs_mode = true;
	} else {
		ACT_LOG_ID_DBG(ALF_STR_i2c_dw_initialize__I2C_HIGH_SPEED_NOT_S, 0);
