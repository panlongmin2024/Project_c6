...
#define ACT_LOG_MODEL_ID ALF_MODEL_TWIHS_SAM
...
static int twihs_sam_configure(struct device *dev, u32_t config)
{
	...
	if (!(config & I2C_MODE_MASTER)) {
		ACT_LOG_ID_ERR(ALF_STR_twihs_sam_configure__MASTER_I2C_MODE_IS_E, 0);
		return -EIO;
	}

	if (config & I2C_ADDR_10_BITS) {
		ACT_LOG_ID_ERR(ALF_STR_twihs_sam_configure__I2C_10BIT_ADDRESSING, 0);
		ACT_LOG_ID_ERR(ALF_STR_twihs_sam_configure__PLEASE_SUBMIT_A_PATC, 0);
		return -EIO;
	}
	...
	case I2C_SPEED_FAST:
		...
		break;
	default:
		ACT_LOG_ID_ERR(ALF_STR_twihs_sam_configure__UNSUPPORTED_I2C_SPEE, 0);
		return -EIO;
	}
