...
#define ACT_LOG_MODEL_ID ALF_MODEL_GPIO_SCH
...
static void _gpio_sch_manage_callback(struct device *dev)
{
	...
	if (!sys_slist_is_empty(&gpio->callbacks) && gpio->cb_enabled) {
		if (!gpio->poll) {
			ACT_LOG_ID_DBG(ALF_STR__gpio_sch_manage_callback__STARTING_SCH_GPIO_PO, 0);
			gpio->poll = 1;
...
static int gpio_sch_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_gpio_sch_init__SCH_GPIO_INTEL_DRIVE, 1, dev);
