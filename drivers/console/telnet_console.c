...
#define ACT_LOG_MODEL_ID ALF_MODEL_TELNET_CONSOLE
			   ...
			   int status, void *token, void *user_data)
{
	if (status) {
		...
		ACT_LOG_ID_ERR(ALF_STR_telnet_sent_cb__COULD_NOT_SENT_LAST_, 0);
	}
}
...
static inline bool telnet_handle_command(struct net_pkt *pkt)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_telnet_handle_command__GOT_A_COMMAND_UUU, 3, cmd->iac, cmd->op, cmd->opt);
...
{
	if (error) {
		ACT_LOG_ID_ERR(ALF_STR_telnet_accept__ERROR_D, 1, error);
		goto error;
	}

	if (client_cnx) {
		ACT_LOG_ID_WRN(ALF_STR_telnet_accept__A_TELNET_CLIENT_IS_A, 0);
		goto error;
	}
				...
				struct sockaddr *addr, socklen_t addrlen)
{
	if (net_context_get(family, SOCK_STREAM, IPPROTO_TCP, ctx)) {
		ACT_LOG_ID_ERR(ALF_STR_telnet_setup_server__NO_CONTEXT_AVAILABLE, 0);
		goto error;
	}
	...
	if (net_context_listen(*ctx, 0)) {
		ACT_LOG_ID_ERR(ALF_STR_telnet_setup_server__CANNOT_LISTEN_ON, 0);
		goto error;
	}

	if (net_context_accept(*ctx, telnet_accept, K_NO_WAIT, NULL)) {
		ACT_LOG_ID_ERR(ALF_STR_telnet_setup_server__CANNOT_ACCEPT, 0);
		goto error;
	}
...
static int telnet_console_init(struct device *arg)
{
	...
	ACT_LOG_ID_INF(ALF_STR_telnet_console_init__TELNET_CONSOLE_INITI, 0);
