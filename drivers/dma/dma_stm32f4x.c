...
#define ACT_LOG_MODEL_ID ALF_MODEL_DMA_STM32F4X
#include <stdio.h>
#include <string.h>
...
{
	ACT_LOG_ID_INF(ALF_STR_dma_stm32_dump_reg__USING_STREAM_DN, 1, id);
...
static void dma_stm32_irq_handler(void *arg, u32_t id)
{
	...
	if ((irqstatus & DMA_STM32_TCI) && (config & DMA_STM32_SCR_TCIE)) {
		...
		ACT_LOG_ID_ERR(ALF_STR_dma_stm32_irq_handler__INTERNAL_ERROR_IRQ_S, 1, irqstatus);
				  ...
				  u32_t id)
{
	...
	for (;;) {
		...
		k_sleep(K_SECONDS(5));
		if (count++ > (5 * 1000) / 50) {
			ACT_LOG_ID_ERR(ALF_STR_dma_stm32_disable_stream__DMA_ERROR_STREAM_IN_, 0);
			return -EBUSY;
		}
	}
