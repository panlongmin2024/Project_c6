...
#define ACT_LOG_MODEL_ID ALF_MODEL_PWM_MCUX_FTM
			    ...
			    u32_t period_cycles, u32_t pulse_cycles)
{
	...
	if (pwm >= config->channel_count) {
		ACT_LOG_ID_ERR(ALF_STR_mcux_ftm_pin_set__INVALID_CHANNEL, 0);
		return -ENOTSUP;
	}
	...
	if (period_cycles != data->period_cycles) {
		...
		ACT_LOG_ID_DBG(ALF_STR_mcux_ftm_pin_set__PWM_FREQD_CLOCK_FREQ, 2, pwm_freq, clock_freq);

		if (pwm_freq == 0) {
			ACT_LOG_ID_ERR(ALF_STR_mcux_ftm_pin_set__COULD_NOT_SET_UP_PWM, 1, pwm_freq);
			return -EINVAL;
		}
		...
		if (status != kStatus_Success) {
			ACT_LOG_ID_ERR(ALF_STR_mcux_ftm_pin_set__COULD_NOT_SET_UP_PWM, 0);
			return -ENOTSUP;
		}
...
static int mcux_ftm_init(struct device *dev)
{
	...
	if (config->channel_count > ARRAY_SIZE(data->channel)) {
		ACT_LOG_ID_ERR(ALF_STR_mcux_ftm_init__INVALID_CHANNEL_COUN, 0);
		return -EINVAL;
	}
