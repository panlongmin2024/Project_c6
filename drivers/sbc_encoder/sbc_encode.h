...
//#ifndef __CONFIG_CHIP_ATS283A
//int sbc_clz(uint32_t x);
//#else
extern int clz(uint32_t x);
#define sbc_clz(x)  clz(x)
//#endif
void  sbc_calculate_bits(const struct sbc_frame *frame, int (*bits)[8]);
uint8_t sbc_crc8(const uint8_t *data, size_t len);
