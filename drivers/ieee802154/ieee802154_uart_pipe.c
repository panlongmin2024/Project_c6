...
#define ACT_LOG_MODEL_ID ALF_MODEL_IEEE802154_UART_PIPE
...
static u8_t *upipe_rx(u8_t *buf, size_t *off)
{
	...
	if (upipe->rx_len == upipe->rx_off) {
		...
		if (!pkt) {
			ACT_LOG_ID_DBG(ALF_STR_upipe_rx__NO_PKT_AVAILABLE, 0);
			goto flush;
		}

		frag = net_pkt_get_frag(pkt, K_NO_WAIT);
		if (!frag) {
			ACT_LOG_ID_DBG(ALF_STR_upipe_rx__NO_FRAGMENT_AVAILABL, 0);
			goto out;
		}
		...
		if (ieee802154_radio_handle_ack(upipe->iface, pkt) == NET_OK) {
			ACT_LOG_ID_DBG(ALF_STR_upipe_rx__ACK_PACKET_HANDLED, 0);
			goto out;
		}

		ACT_LOG_ID_DBG(ALF_STR_upipe_rx__CAUGHT_A_PACKET_U, 1, upipe->rx_len);
		if (net_recv_data(upipe->iface, pkt) < 0) {
			ACT_LOG_ID_DBG(ALF_STR_upipe_rx__PACKET_DROPPED_BY_NE, 0);
			goto out;
		}
		    ...
		    struct net_buf *frag)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_upipe_tx__08X_U, 2, frag, len);
...
static void upipe_iface_init(struct net_if *iface)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_upipe_iface_init__, 0);
