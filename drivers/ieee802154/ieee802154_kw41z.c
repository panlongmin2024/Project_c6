...
#define ACT_LOG_MODEL_ID ALF_MODEL_IEEE802154_KW41Z
...
static inline void kw41z_wait_for_idle(void)
{
	...
	if (state != KW41Z_STATE_IDLE) {
		ACT_LOG_ID_ERR(ALF_STR_kw41z_wait_for_idle__ERROR_WAITING_FOR_ID, 0);
	}
}
...
static int kw41z_cca(struct device *dev)
{
	...
	if (kw41z_prepare_for_new_state()) {
		ACT_LOG_ID_DBG(ALF_STR_kw41z_cca__CANT_INITIATE_NEW_SE, 0);
		return -EBUSY;
	}
...
static inline void kw41z_rx(struct kw41z_context *kw41z, u8_t len)
{
	...
	if (!pkt) {
		ACT_LOG_ID_ERR(ALF_STR_kw41z_rx__NO_BUF_AVAILABLE, 0);
		goto out;
	}

	frag = net_pkt_get_frag(pkt, K_NO_WAIT);
	if (!frag) {
		ACT_LOG_ID_ERR(ALF_STR_kw41z_rx__NO_FRAG_AVAILABLE, 0);
		goto out;
	}
	...
	if (ieee802154_radio_handle_ack(kw41z->iface, pkt) == NET_OK) {
		ACT_LOG_ID_DBG(ALF_STR_kw41z_rx__ACK_PACKET_HANDLED, 0);
		goto out;
	}
	...
	if (net_recv_data(kw41z->iface, pkt) < 0) {
		ACT_LOG_ID_DBG(ALF_STR_kw41z_rx__PACKET_DROPPED_BY_NE, 0);
		goto out;
	}
		    ...
		    struct net_buf *frag)
{
	...
	if (kw41z_prepare_for_new_state()) {
		ACT_LOG_ID_DBG(ALF_STR_kw41z_tx__CANT_INITIATE_NEW_SE, 0);
		return -EBUSY;
	}

	if (payload_len > KW41Z_PSDU_LENGTH) {
		ACT_LOG_ID_ERR(ALF_STR_kw41z_tx__PAYLOAD_TOO_LONG, 0);
		return 0;
	}
...
static void kw41z_isr(int unused)
{
	...
	if ((irqsts & ZLL_IRQSTS_TMR1IRQ_MASK) &&
		...
		ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__TMR1_TIMEOUT, 0);
		kw41z_tmr1_disable();
		kw41z_disable_seq_irq();
	...
	if ((irqsts & ZLL_IRQSTS_TMR2IRQ_MASK) &&
		...
		ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__TMR2_TIMEOUT, 0);
		kw41z_tmr2_disable();
		atomic_set(&kw41z_context_data.seq_retval, -EBUSY);
		k_sem_give(&kw41z_context_data.seq_sync);
	}
	...
	if ((state != KW41Z_STATE_IDLE) && (irqsts & ZLL_IRQSTS_SEQIRQ_MASK)) {
		...
		switch (state) {
		case KW41Z_STATE_RX:
			ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__RX_SEQ_DONE, 0);
		...
		case KW41Z_STATE_TXRX:
			ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__TXRX_SEQ_DONE, 0);
			kw41z_tmr2_disable();
		case KW41Z_STATE_TX:
			ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__TX_SEQ_DONE, 0);
		...
		case KW41Z_STATE_CCA:
			ACT_LOG_ID_DBG(ALF_STR_kw41z_isr__CCA_SEQ_DONE, 0);
			if (irqsts & ZLL_IRQSTS_CCA_MASK) {
				atomic_set(&kw41z_context_data.seq_retval,
					   -EBUSY);
			} else {
