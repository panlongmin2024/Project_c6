...
#define ACT_LOG_MODEL_ID ALF_MODEL_IEEE802154_NRF5
...
static void nrf5_rx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		ACT_LOG_ID_DBG(ALF_STR_nrf5_rx_thread__WAITING_FOR_FRAME, 0);
		k_sem_take(&nrf5_radio->rx_wait, K_FOREVER);

		ACT_LOG_ID_DBG(ALF_STR_nrf5_rx_thread__FRAME_RECEIVED, 0);

		pkt = net_pkt_get_reserve_rx(0, K_NO_WAIT);
		if (!pkt) {
			ACT_LOG_ID_ERR(ALF_STR_nrf5_rx_thread__NO_PKT_AVAILABLE, 0);
			goto out;
		}
		...
		if (!frag) {
			ACT_LOG_ID_ERR(ALF_STR_nrf5_rx_thread__NO_FRAG_AVAILABLE, 0);
			goto out;
		}
		...
		if (ack_result == NET_OK) {
			ACT_LOG_ID_DBG(ALF_STR_nrf5_rx_thread__ACK_PACKET_HANDLED, 0);
			goto out;
		}
		...
		if (net_recv_data(nrf5_radio->iface, pkt) < 0) {
			ACT_LOG_ID_DBG(ALF_STR_nrf5_rx_thread__PACKET_DROPPED_BY_NE, 0);
			goto out;
		}
...
static int nrf5_cca(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_cca__CCA_D, 1, nrf5_radio->channel_ed);
...
static int nrf5_set_channel(struct device *dev, u16_t channel)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_set_channel__U, 1, channel);
...
static int nrf5_set_pan_id(struct device *dev, u16_t pan_id)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_set_pan_id__0XX, 1, pan_id);
	return 0;
}
...
static int nrf5_set_short_addr(struct device *dev, u16_t short_addr)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_set_short_addr__0XX, 1, short_addr);
	return 0;
}
...
static int nrf5_set_txpower(struct device *dev, s16_t dbm)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_set_txpower__D, 1, dbm);
		   ...
		   struct net_buf *frag)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_tx__08X_U, 2, payload, payload_len);
	...
	if (!nrf_drv_radio802154_transmit(nrf5_radio->tx_psdu,
					  nrf5_radio->channel,
					  nrf5_radio->txpower)) {
		ACT_LOG_ID_ERR(ALF_STR_nrf5_tx__CANNOT_SEND_FRAME, 0);
		return -EIO;
	}
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_tx__RESULT_D, 1, nrf5_data.tx_success);
...
static int nrf5_stop(struct device *dev)
{
	...
	if (!nrf_drv_radio802154_sleep()) {
		ACT_LOG_ID_ERR(ALF_STR_nrf5_stop__ERROR_WHILE_STOPPING, 0);
		return -EIO;
	}

	ACT_LOG_ID_DBG(ALF_STR_nrf5_stop__NRF5_802154_RADIO_ST, 0);
...
static int nrf5_init(struct device *dev)
{
	...
	ACT_LOG_ID_INF(ALF_STR_nrf5_init__NRF5_802154_RADIO_IN, 0);
...
static void nrf5_iface_init(struct net_if *iface)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_nrf5_iface_init__, 0);
