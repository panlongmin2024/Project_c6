...
static void spi_acts_set_clk(const struct acts_spi_config *cfg, u32_t freq_khz)
{
	 ...
	 */
	//sys_write32((sys_read32(CMU_SPICLK) & ~(0xff << 16)) | (0x85 << 16), CMU_SPICLK);
	k_busy_wait(100);
#endif
}
				  ...
				  void (*callback)(struct device *, u32_t, int))
{
	...
	if (is_tx) {
		...
		dma_cfg.dest_data_size = (spi->ctrl & SPI_CTL_FIFO_WIDTH_32BIT) ? 4 : 1;
		dma_cfg.dest_burst_length = (spi->ctrl & SPI_CTL_FIFO_WIDTH_32BIT) ? 1 : 0;
	} else {
		dma_cfg.dma_slot = cfg->rxdma_id;
		dma_cfg.channel_direction = PERIPHERAL_TO_MEMORY;
		dma_block_cfg.source_address = (u32_t)&spi->rxdat;
		dma_block_cfg.dest_address = (u32_t)buf;
		dma_cfg.source_data_size = (spi->ctrl & SPI_CTL_FIFO_WIDTH_32BIT) ? 4 : 1;
		dma_cfg.source_burst_length = (spi->ctrl & SPI_CTL_FIFO_WIDTH_32BIT) ? 1 : 0;
	}
		...
		const u8_t *tx_buf, u8_t *rx_buf, s32_t len)
{
	...
	spi->ctrl = (spi->ctrl & ~SPI_CTL_WR_MODE_MASK) |
			...
			SPI_CTL_TX_DRQ_EN | SPI_CTL_RX_DRQ_EN | (1<<15);
		...
		const u8_t *wbuf, s32_t len)
{
	...
	if(spi->ctrl & SPI_CTL_FIFO_WIDTH_32BIT)
		real_size = 4;
	/* switch to write mode */
	//spi->bc = len;
	spi->ctrl = (spi->ctrl & ~SPI_CTL_WR_MODE_MASK) | SPI_CTL_WR_MODE_WRITE | (1<<15);
	k_busy_wait(100);
	while (tx_len < len / real_size) {
		if(!(spi->status & SPI_STATUS_TX_FULL)) {
			//printk("%x\n",*(u32_t*)wbuf);
			spi->txdat = *(u32_t*)wbuf;
			wbuf += real_size;
			tx_len++;
		}
	}
		...
		u8_t *rbuf, s32_t len)
{
	...
	k_busy_wait(100);
	spi->bc = len;
	spi->ctrl |= SPI_CTL_WR_MODE_READ;
	while (rx_len < len / real_size) {
		if(!(spi->status & SPI_STATUS_RX_EMPTY)) {
			*(u32_t*)rbuf = spi->rxdat;
			rbuf += real_size;
			rx_len++;
		}
	}
...
int spi_acts_transfer_data(const struct acts_spi_config *cfg, struct acts_spi_data *data)
{
	...
	spi->ctrl = (spi->ctrl & ~(SPI_CTL_WR_MODE_MASK | (1<<15) | SPI_CTL_RX_FIFO_EN)) | SPI_CTL_WR_MODE_DISABLE;
			    ...
			    struct spi_config *config)
{
	...
	if (word_size == 8)
		...
		ctrl |= SPI_CTL_FIFO_WIDTH_32BIT | (1<<25);
	}
	else
		ctrl |= SPI_CTL_FIFO_WIDTH_8BIT;
