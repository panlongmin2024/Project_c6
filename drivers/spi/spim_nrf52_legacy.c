...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPIM_NRF52_LEGACY
				...
				struct spi_config *spi_config)
{
	...
	if (SPI_WORD_SIZE_GET(flags) != 8) {
		ACT_LOG_ID_ERR(ALF_STR_spim_nrf52_configure__UNSUPPORTED_WORD_SIZ, 0);
		return -EINVAL;
	}

	if (flags & SPI_MODE_LOOP) {
		ACT_LOG_ID_ERR(ALF_STR_spim_nrf52_configure__LOOPBACK_UNSUPPORTED, 0);
		return -EINVAL;
	}
...
static int spim_nrf52_slave_select(struct device *dev, u32_t slave)
{
	...
	if (config->psel.ss[slave] == SS_UNUSED) {
		ACT_LOG_ID_ERR(ALF_STR_spim_nrf52_slave_select__SLAVE_D_IS_NOT_CONFI, 1, slave);
		return -EINVAL;
	}
	...
	for (i = 0; i < sizeof(config->psel.ss); i++) {
		if (config->psel.ss[i] != SS_UNUSED) {
			...
			spim_nrf52_csn(data->gpio_port, config->psel.ss[i],
				       false);
			ACT_LOG_ID_DBG(ALF_STR_spim_nrf52_init__CSDDN, 2, i, config->psel.ss[i]);
		}
	}
