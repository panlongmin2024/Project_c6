...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPI_MCUX_DSPI
...
static int spi_mcux_configure(struct device *dev, struct spi_config *spi_config)
{
	...
	if (!clock_freq) {
		ACT_LOG_ID_ERR(ALF_STR_spi_mcux_configure__GOT_FREQUENCY_OF_0, 0);
		return -EINVAL;
	}
		...
		void *rx_buf, u32_t rx_buf_len)
{
	...
	if (tx_buf_len == rx_buf_len) {
		...
		ACT_LOG_ID_DBG(ALF_STR_spi_mcux_transceive__USING_TX_AND_RX_BUFF, 0);
		...
		transfer.rxData = rx_buf;
		transfer.dataSize = rx_buf_len;
		ACT_LOG_ID_DBG(ALF_STR_spi_mcux_transceive__USING_RX_BUFFER_DIRE, 0);
		...
		ACT_LOG_ID_DBG(ALF_STR_spi_mcux_transceive__USING_TX_BUFFER_DIRE, 0);
		...
		ACT_LOG_ID_DBG(ALF_STR_spi_mcux_transceive__USING_LOCAL_BUFFER_F, 0);
		...
		ACT_LOG_ID_ERR(ALF_STR_spi_mcux_transceive__LOCAL_BUFFER_TOO_SMA, 0);
		return -EINVAL;
	}
	...
	if (status != kStatus_Success) {
		ACT_LOG_ID_ERR(ALF_STR_spi_mcux_transceive__TRANSFER_COULD_NOT_S, 0);
		return -EIO;
	}
	...
	if (data->callback_status != kStatus_Success) {
		ACT_LOG_ID_ERR(ALF_STR_spi_mcux_transceive__TRANSFER_COULD_NOT_C, 0);
		return -EIO;
	}
...
static int spi_mcux_init(struct device *dev)
{
	...
	if (error) {
		ACT_LOG_ID_ERR(ALF_STR_spi_mcux_init__COULD_NOT_CONFIGURE, 0);
		return error;
	}
