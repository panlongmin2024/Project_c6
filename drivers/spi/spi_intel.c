...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPI_INTEL
...
static void pull_data(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_pull_data__PULLED_D_TOTAL_D, 2,	cnt, spi->received);
}
...
static void push_data(struct device *dev)
{
	...
	while ((status = read_sssr(spi->regs)) & INTEL_SPI_SSSR_TNF) {
		...
		cnt++;
		ACT_LOG_ID_DBG(ALF_STR_push_data__PUSHING_1_BYTE_TOTAL, 1, cnt);
		write_ssdr(data, spi->regs);
		spi->transmitted++;
	}

	ACT_LOG_ID_DBG(ALF_STR_push_data__PUSHED_D_TOTAL_D, 2, cnt, spi->transmitted);
				...
				struct spi_config *config)
{
	...
	if (test_bit_sscr0_sse(spi->regs) && test_bit_sssr_bsy(spi->regs)) {
		ACT_LOG_ID_DBG(ALF_STR_spi_intel_configure__SPI_INTEL_CONFIGURE_, 0);
		return -EBUSY;
	}
				...
				void *rx_buf, u32_t rx_buf_len)
{
	...
	/* Check status */
	if (test_bit_sscr0_sse(spi->regs) && test_bit_sssr_bsy(spi->regs)) {
		ACT_LOG_ID_DBG(ALF_STR_spi_intel_transceive__SPI_INTEL_TRANSCEIVE, 0);
		return -EBUSY;
	}
...
void spi_intel_isr(void *arg)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_intel_isr__SPI_INTEL_ISR_08X, 1, dev);
...
static inline int spi_intel_setup(struct device *dev)
{
	...
	if (!pci_bus_scan(&spi->pci_dev)) {
		ACT_LOG_ID_DBG(ALF_STR_spi_intel_setup__COULD_NOT_FIND_DEVIC, 0);
		return 0;
	}
...
int spi_intel_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_intel_init__SPI_INTEL_DRIVER_INI, 1, dev);
...
static int spi_intel_suspend(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_intel_suspend__SPI_INTEL_SUSPEND_08, 1, dev);
...
static int spi_intel_resume_from_suspend(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_intel_resume_from_suspend__SPI_INTEL_RESUME_08X, 1, dev);
