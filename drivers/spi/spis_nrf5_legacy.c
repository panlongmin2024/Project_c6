...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPIS_NRF5_LEGACY
#include <sys_io.h>
#include <board.h>
#include <init.h>
#include <gpio.h>
...
static int spis_nrf5_configure(struct device *dev, struct spi_config *config)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spis_nrf5_configure__SPI_SLAVE_DRIVER_CON, 0);
			 ...
			 u32_t tx_buf_len, void *rx_buf, u32_t rx_buf_len)
{
	...
	if (!tx_buf || !is_buf_in_ram(tx_buf)) {
		ACT_LOG_ID_ERR(ALF_STR_spis_nrf5_transceive__INVALID_TX_BUF_08X, 1, tx_buf);
		return -EINVAL;
	}
	if (!rx_buf || !is_buf_in_ram(rx_buf)) {
		ACT_LOG_ID_ERR(ALF_STR_spis_nrf5_transceive__INVALID_RX_BUF_08X, 1, rx_buf);
		return -EINVAL;
	}
	...
	if (spi_regs->SEMSTAT == 1) {
		...
		ACT_LOG_ID_ERR(ALF_STR_spis_nrf5_transceive__CANT_GET_SEM_UNFINIS, 0);
		return -EIO;
	}
...
static int spis_nrf5_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spis_nrf5_init__SPI_SLAVE_DRIVER_INI, 1, dev);
