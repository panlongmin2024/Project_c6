...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPI_DW
			    ...
			    struct spi_config *config)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_dw_configure__08X_PREV_08X, 2, config, spi->ctx.config);
		      ...
		      struct k_poll_signal *signal)
{
	...
	if (test_bit_ssienr(info->regs) || test_bit_sr_busy(info->regs)) {
		ACT_LOG_ID_DBG(ALF_STR_transceive__CONTROLLER_IS_BUSY, 0);
		return -EBUSY;
	}
