...
#define ACT_LOG_MODEL_ID ALF_MODEL_SPI_DW_LEGACY
...
static void push_data(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_push_data__PUSHED_D, 1, DBG_COUNTER_RESULT());
}
...
static void pull_data(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_pull_data__PULLED_D, 1, DBG_COUNTER_RESULT());
}
			    ...
			    struct spi_config *config)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_dw_configure__08X_0XX_08X, 3, dev, info->regs, config);

	/* Check status */
	if (!_spi_dw_is_controller_ready(dev)) {
		ACT_LOG_ID_DBG(ALF_STR_spi_dw_configure__CONTROLLER_IS_BUSY, 0);
		return -EBUSY;
	}
...
static int spi_dw_slave_select(struct device *dev, u32_t slave)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_dw_slave_select__08X_D, 2, dev, slave);
			     ...
			     void *rx_buf, u32_t rx_buf_len)
{
	...
	/* Check status */
	if (!_spi_dw_is_controller_ready(dev)) {
		ACT_LOG_ID_DBG(ALF_STR_spi_dw_transceive__CONTROLLER_IS_BUSY, 0);
		return -EBUSY;
	}
...
int spi_dw_init(struct device *dev)
{
	...
	ACT_LOG_ID_DBG(ALF_STR_spi_dw_init__DESIGNWARE_SPI_DRIVE, 1, dev);
