...
struct acts_battery_info {
	...
	struct report_deb_ctr report_ctr[3];
	...
	u16_t iic_bat_cap;
	u16_t iic_bat_remain_charge_time;
	int iic_bat_temperature;
...
static int __battery_get_voltage(struct acts_battery_info *bat)
{
	...
	adc_val = 0x2b0;//bat->iic_bat_voltage;
#else
...
static int battery_get_charge_status(struct acts_battery_info *bat, const void *cfg)
{
	...
	u32_t sink_status = 0;
	static u32_t charge_full_cnt = 0;
	...
	//if(!hw_info){
		...
		sink_status = dc_power_in_status_read();
	}
  #endif
	if (sink_status)
#else	
	...
	#endif
		//if(bat->iic_bat_cap == 100 && bat->iic_bat_remain_charge_time == 0xffff){
#ifdef CONFIG_BUILD_PROJECT_HM_DEMAND_CODE		
	//	if(!hw_info)
		{
		//if(1){
			...
			// SYS_LOG_INF("[%d], charge_full_cnt:%d\n ", __LINE__, charge_full_cnt);
...
//		else
		...
		// 	if(bat->iic_bat_cap >= 100 /*&& (bat->iic_bat_remain_charge_time <= 20 || bat->iic_bat_remain_charge_time == 0xffff)*/){
		// 		charge_full_cnt++;
		// 	}
		// 	else{
		// 		charge_full_cnt = 0;
...
#ifdef CONFIG_WLT_MODIFY_BATTERY_DISPLAY	
	else if(sys_pm_get_power_5v_status() == 2){
		charge_status = POWER_SUPPLY_STATUS_EXT_CHARGING;
		charge_full_cnt = 0;
	}
#endif	
	else
...
void battery_acts_voltage_poll(struct acts_battery_info *bat)
{
	...
	*/
	...
	if ((temp != 0xff) && (temp != bat->cur_temperature))
	{
		bat->cur_temperature = temp;
	}
		...
		&& (bat->cur_chg_status != POWER_SUPPLY_STATUS_UNKNOWN))
	{
		...
		bat->report_ctr[INDEX_CAP].times = 60;   // unit 100ms
		bat->report_ctr[INDEX_CAP].step = 1;      //   %		
		...
		bat->report_ctr[INDEX_TEMP].times = 60;   // unit 100ms
		bat->report_ctr[INDEX_TEMP].step = 1;      //   %		
		...
		{
			printk("zth debug: %s:%d; REPORT VAL:0x%x\n",__func__,__LINE__, bat->cur_voltage); 
		...
		{
			printk("zth debug: %s:%d; REPORT CAP:0x%x\n",__func__,__LINE__, bat->cur_cap); 
...
#ifdef CONFIG_WLT_MODIFY_BATTERY_DISPLAY
		if ( bat_report_debounce(bat->last_temperature_report,    bat->cur_temperature, &bat->report_ctr[INDEX_TEMP])
			|| (bat->last_temperature_report == BAT_TEMP_RESERVE))
		{
			printk("zth debug: %s:%d; REPORT TEMP:0x%x\n",__func__,__LINE__, bat->cur_temperature); 
			para.temperature = bat->cur_temperature;
			if (bat_charge_callback(BAT_CHG_EVENT_TEMP_CHANGE, &para, bat))
				bat->last_temperature_report = bat->cur_temperature;
		}		
	}
#endif
...
void dc5v_status_check(struct acts_battery_info *bat)
{
	...
	//printk("zth debug: %s:%d; DC:0x%x\n",__func__,__LINE__, dc_power_in_status_read()); 
...
#ifdef CONFIG_WLT_MODIFY_BATTERY_DISPLAY	
	case POWER_SUPPLY_PROP_BAT_TEMP:
		val->intval = bat->iic_bat_temperature;
		return 0;	
#endif
				       ...
				       union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		bat->iic_bat_voltage = val->intval;
		// printk("zth debug: %s:%d; bat:0x%x\n",__func__,__LINE__, bat->iic_bat_voltage); 
		break;

	case POWER_SUPPLY_PROP_CAPACITY:
		bat->iic_bat_cap = val->intval;
		// printk("zth debug: %s:%d; cap:0x%x\n",__func__,__LINE__, bat->iic_bat_cap); 
		break;	

#ifdef CONFIG_WLT_MODIFY_BATTERY_DISPLAY		
	case POWER_SUPPLY_PROP_REMAIN_CHARGE_TIME:
		bat->iic_bat_remain_charge_time = val->intval;
		// printk("zth debug: %s:%d; remain_charge_time:0x%x\n",__func__,__LINE__, bat->iic_bat_remain_charge_time); 
		break;		

	case POWER_SUPPLY_PROP_BAT_TEMP:
		bat->iic_bat_temperature = val->intval;
		
		SYS_LOG_INF( "[%d] bat_temp:%d, cap: %d%%, re_time: %d \n ", __LINE__, bat->iic_bat_temperature, \
								bat->iic_bat_cap, bat->iic_bat_remain_charge_time); 
		break;
#endif		
...
static int battery_acts_init(struct device *dev)
{
	...
	bat->iic_bat_voltage = 0xfa0;//4000mv;
#ifdef CONFIG_WLT_MODIFY_BATTERY_DISPLAY
	bat->iic_bat_cap = 100;
	bat->iic_bat_temperature = 250;
#endif	
	bat_voltage_record_reset(bat);
	adc_enable(bat->adc_dev);
