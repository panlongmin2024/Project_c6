 ...
 *      wuyufan   2020-10-29-下午9:20:27             1.0             build this file
 ********************************************************************************/
/*!
 * \file     soc_export_rom.h
 * \brief
 * \author
 * \version  1.0
 * \date  2020-10-29-下午9:20:27
 *******************************************************************************/

#ifndef SOC_EXPORT_ROM_H_
#define SOC_EXPORT_ROM_H_
...
typedef int (*api_sbc_decoder_ops)(void *handle, unsigned int cmd, unsigned int args);

typedef int (*api_sbc_encoder_ops)(void *handle, unsigned int cmd, unsigned int args);

typedef int (*api_printf)(const char *fmt, ...);

typedef void (*api_io_write)(unsigned int pin, unsigned int value);

typedef void  (*api_io_out)(int pin, int level, uint32_t delay_time_us);
...
typedef unsigned int (*api_crc32)(unsigned int crc_initial, unsigned char *buffer, unsigned int buf_len);

typedef int (*api_vsprintf)(char * s, const char * format,
	     va_list vargs);

typedef int (*api_prf)(int (*func)(), void *dest, const char *format, va_list vargs);

typedef int (*api_vsnprintf_std)(char* buf, unsigned int size, const char* fmt, va_list args);
...
//该结构体定义BROM模块对外提供一些函数的接口表，其它模块通过该接口表直接可以访问
//BROM的一些函数
//接口表定义在BROM起始一段空间，这样即使后面BROM代码有所调整，固件端程序也不需要修改
typedef struct brom_operations
{
    ...
    api_crc32                                           p_crc32;
    api_vsprintf                                        p_vsprintf;
    api_prf                                             p_prf;
	api_vsnprintf_std                                   p_vsnprintf_std;
}brom_operations_t;
