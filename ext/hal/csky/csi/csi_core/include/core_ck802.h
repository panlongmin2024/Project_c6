...
#define _IP_PRIO_IDX(IRQn)       (   (((uint32_t)(int32_t)(IRQn))                >>    2UL)      )
...
{
    VIC->IPR[_IP_PRIO_IDX(IRQn)] = ((uint32_t)(VIC->IPR[_IP_PRIO_IDX(IRQn)]  & ~(0xFFUL << _BIT_SHIFT(IRQn))) |
...
{
    return ((uint32_t)(((VIC->IPR[_IP_PRIO_IDX(IRQn)] >> _BIT_SHIFT(IRQn)) & (uint32_t)0xFFUL) >> (8U - __VIC_PRIO_BITS)));
}
...
__STATIC_INLINE unsigned int csi_get_sp(void)
{
    unsigned int result;
...
__STATIC_INLINE void csi_enable_all_irq(void)
{
    __asm__ volatile("psrset ee,ie");
    __asm__ __volatile ("nop" ::: "memory");
}

__STATIC_INLINE int clz(unsigned int a)
{
 ...
 */
__STATIC_INLINE int fls(unsigned int a)
{
 ...
 */
__STATIC_INLINE int ffs(unsigned int a)
{
    if (!a) return 0;
...
__STATIC_INLINE int bswap32(unsigned int a)
{
