...
#define MAX_HOLD_KEY_SUPPORT        5
#define MAX_MUTIPLE_CLICK_KEY_SUPPORT        4

#define LONG_PRESS_TIMER 3
#define SUPER_LONG_PRESS_TIMER             33    /* time */
#define SUPER_LONG_PRESS_5S_TIMER         83   /* time */
#define SUPER_LONG_PRESS_10S_TIMER         166   /* time */
#define QUICKLY_CLICK_DURATION 500 /* ms */
...
void key_event_handle(struct device *dev, struct input_value *val)
{
	...
	case KEY_VALUE_UP:
	{
		...
		if (input_manager->press_type == KEY_TYPE_LONG_DOWN){
			input_manager->press_type = KEY_TYPE_SHORT_LONG_UP;
		}else if (input_manager->press_type == KEY_TYPE_LONG_DOWN
			 || input_manager->press_type == KEY_TYPE_LONG
			 || input_manager->press_type == KEY_TYPE_LONG10S
			 || input_manager->press_type == KEY_TYPE_LONG5S) {
	...
	case KEY_VALUE_DOWN:
	{
		if (val->code != input_manager->press_code) {
			...
			if (input_manager->press_timer >= SUPER_LONG_PRESS_10S_TIMER) {
				if (input_manager->press_type != KEY_TYPE_LONG10S) {
					input_manager->press_type = KEY_TYPE_LONG10S;
					need_report = true;
				}
			} else if (input_manager->press_timer >= SUPER_LONG_PRESS_5S_TIMER) {
				if (input_manager->press_type != KEY_TYPE_LONG5S) {
					input_manager->press_type = KEY_TYPE_LONG5S;
					need_report = true;
				}
				...
				if (input_manager->press_type != KEY_TYPE_LONG_DOWN) {
					...
					if (is_support_hold(input_manager->press_code)) {
						...
						os_delayed_work_submit(&input_manager->hold_key_work, 800);
					}
					need_report = true;
				}
			}
	...
	SYS_LOG_INF("Plm debnug:val=0x%x type:0x%x report_type:0x%x time:%d need:%d lock:%d\n",
				val->code,input_manager->press_type,input_manager->report_key_value,input_manager->press_timer,need_report,input_manager_islock());
	if (need_report) {
...
#ifdef CONFIG_BUILD_PROJECT_HM_DEMAND_CODE
void input_manager_disable_key(u32_t key_code)
{
	unsigned int key;
	key = irq_lock();
	for(int i = 0;i < 10;i++){
		if(diable_key[i] == key_code){
			irq_unlock(key);
			return; 
		}
	}
...
void input_manager_release_disable_key(u32_t key_code)
{
	unsigned int key;
	key = irq_lock();
	for(int i = 0;i < 10;i++){
...
void input_manager_clean_disable_key(void)
{
	unsigned int key;
	key = irq_lock();
	for(int i = 0;i < 10;i++){
