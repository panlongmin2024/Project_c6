...
static u8_t usb_hotplug_stack[CONFIG_USB_HOTPLUG_STACKSIZE] __aligned(4);
...
static int usb_controler_high_state = 0;
static int usb_controler_high_state_status = 1;

void usb_controler_enable(int enable)
{
	usb_controler_high_state = enable;
}
#endif
...
#endif
#ifdef CONFIG_BUILD_PROJECT_HM_DEMAND_CODE
	if((usb_controler_high_state == 0) && (!run_mode_is_demo())){
		otg_state = OTG_STATE_A_IDLE;
		if(usb_controler_high_state_status != 0){
			usb_phy_enter_high_state();
			usb_controler_high_state_status = 0;
		}
		return ;
	}
	usb_controler_high_state_status = 1;
#endif	
