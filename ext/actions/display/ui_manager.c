...
int ui_manager_dispatch_key_event(u32_t key_event)
{
	...
	else if(key_val == KEY_VOLUMEUP){
		wlt_simulation_test_handle(true);
	}
	else if(key_val == KEY_VOLUMEDOWN){
		wlt_simulation_test_handle(false);
	}							
#endif			
	}
#endif	
	SYS_SLIST_FOR_EACH_CONTAINER(&ui_manager->view_list, view, node) {
		...
		if(!os_is_free_msg_enough()){
			SYS_LOG_INF("drop key msg ... %d", msg_pool_get_free_msg_num());
			return -ENOMEM;
		}

		while (view_key_map[index].key_val != KEY_RESERVED) {

		//	printk("Totti debug:%s:%d: %d\n", __func__, __LINE__, view_key_map[index].key_val);	
			...
			//		new_msg.value = key_val;								// Totti modify on 2023/12/02
					result = send_async_msg(view->app_id, &new_msg);
					return result;
				}
			}
