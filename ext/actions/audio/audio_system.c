...
int audio_system_set_stream_volume(int stream_type, int volume)
{
	...
	if (stream_type == AUDIO_STREAM_LE_AUDIO) {
	    audio_system->le_volume = volume;
	    _audio_system_save_volume(CFG_LE_VOLUME, volume);
	}
...
void audio_system_clear_volume(void){
	...
	property_set(CFG_LE_VOLUME, NULL,0);
...
int aduio_system_init(void)
{
	...
	_audio_system_get_volume_config(CFG_LE_VOLUME, &audio_system->le_volume);
