...
static bool app_manager_check_stack_safe(struct app_info *appinfo)
{
	...
	SYS_SLIST_FOR_EACH_NODE_SAFE(&global_app_list, node, tmp) {
		...
		if (app->stackptr == appinfo->stackptr) {
			SYS_LOG_ERR("app->name %s used stack %p\n", app->name, appinfo->stackptr);
			return false;
		}
...
wait:
	if (os_sem_take(&callback_sem, OS_MSEC(6000)) != 0) {
		SYS_LOG_WRN("app %s exit timeout\n", app_name);
		if (app_manager_get_apptid(app_name)) {
			goto wait;
		}
	}

	SYS_LOG_INF("app %s exited\n", app_name);
	return true;
}
...
bool app_manager_active_app(char *app_name)
{
	...
	/* do nothing if current app is to be actived*/
	if (target_app_entry == actived_app)	{
		SYS_LOG_INF("current app is your need\n");
		goto exit;
	}
...
bool app_manager_suspend_app(char * app_name)
{
	struct app_msg msg = {0};
	os_sem callback_sem;
	struct app_info *appinfo = app_manager_get_app_info(app_name);
	...
	if (os_sem_take(&callback_sem, 10000) != 0) {
		SYS_LOG_WRN("app %s exit timeout.\n", appinfo->name);
	}
...
bool app_manager_resume_app(char * app_name)
{
	struct app_msg msg = {0};
	os_sem callback_sem;
	struct app_info *appinfo = app_manager_get_app_info(app_name);
	...
	if (os_sem_take(&callback_sem, 10000) != 0) {
		SYS_LOG_WRN("app %s resume timout.\n", appinfo->name);
	}
