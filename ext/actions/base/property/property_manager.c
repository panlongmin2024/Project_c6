...
int property_set_factory_no_use_rw_region(const char *key, char *value, int value_len)
{
	int ret = -ENOENT;
#ifdef CONFIG_NVRAM_CONFIG
	ret = nvram_config_set_factory_no_use_rw_region(key, value, value_len);
#endif
	return ret;
}
...
static uint8_t property_factory_reset_flag;

bool get_property_factory_reset_flag(void)
{
	return property_factory_reset_flag;
}

void set_property_factory_reset_flag(bool flag)
{
	if (property_factory_reset_flag == flag)
	{
		SYS_LOG_ERR("already set:%d\n", flag);
		return;
	}

	SYS_LOG_INF("set:%d\n", flag);
	
	property_factory_reset_flag = flag;
}
