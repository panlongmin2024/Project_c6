 ...
 *      wuyufan   2018-8-11-下午2:22:06             1.0             build this file
 ********************************************************************************/
/*!
 * \file     cbuf.c
 * \brief    circulation buffer code
 * \author
 * \version  1.0
 * \date  2018-8-11-下午2:22:06
 *******************************************************************************/
#include <kernel.h>
#include <cbuf.h>
#include <string.h>
#ifndef CONFIG_USE_ROM_CBUF
...
void cbuf_copy_write_ptr(cbuf_t *dest, uint32_t write_ptr, uint32_t len)
{
	uint32_t flags;

	flags = irq_lock();

	dest->write_ptr = write_ptr;
    dest->length += len;
