...
{
	printk("%s%s %p(t:%d,s:%d):\n", line_prefix, name, stream, stream->type, stream->state);
	printk("%s\trofs=0x%x, wofs=0x%x, size=0x%x, length=0x%x, space=0x%x\n",
		line_prefix, stream->rofs, stream->wofs, stream->total_size, 
		stream_get_length(stream), stream_get_space(stream));

	//Fixme: Extend stream options api to do concrete stream dump.
	if (stream->type == RINGBUFFER_STREAM_IMAG_NUM) {
		int *rb = (int*)(stream->data);
		if (NULL != rb) {
			acts_ringbuf_dump((struct acts_ringbuf *)(*rb), "buffer", "\t\t");
		}
	}
}
