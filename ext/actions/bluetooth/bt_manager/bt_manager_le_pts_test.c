...
typedef struct
{
	...
	struct bt_le_ext_adv *pts_adv;
	struct bt_le_ext_adv_cb pts_adv_cb;
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	struct bt_le_per_adv_subevent_data_params subevts_params[2];
	struct net_buf_simple *resp_buf;
...
static void le_addr_output(bt_addr_le_t *le_addr)
{
	SYS_LOG_INF("addr type:%d,addr val:%02x %02x %02x %02x %02x %02x\n",
		le_addr->type,
		le_addr->a.val[0],le_addr->a.val[1],le_addr->a.val[2],
		le_addr->a.val[3],le_addr->a.val[4],le_addr->a.val[5]);
}
...
static void pts_connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (0 != err)
	{
		...
		if (type == BT_CONN_TYPE_LE) {
			bt_pts_clear_keys(&s_ctx.peer_addr);
...
static int pts_get_attr_handle_by_uuid(int argc, char *argv[])
{
	if (argc < 2)
	{
		SYS_LOG_INF("Used: pts_le get_attr_handle_by_uuid 0xXXXX\n");
		return -EINVAL;
	}

	uint16_t value = strtoul(argv[1], NULL, 16);

	uint16_t handle = btif_pts_test_get_attr_by_uuid(value);
...
void pts_le_clear_keys(void)
{
	uint8_t addr[13] = {0};
	hex_to_str(addr, s_ctx.peer_addr.a.val, PTS_MAC_ADDR_SIZE);
	uint8_t is_clear = bt_pts_clear_keys(&s_ctx.peer_addr);
...
#define CONFIG_BT_PAWR_PAST
#ifdef CONFIG_BT_PAWR_PAST

uint8_t pts_subevt_data[5] = {4, 0, 2, 3, 4};
uint8_t pts_pa_data[19] = {0x04,
		0x00, 0x18, 0x01, 0x18,
		0x50, 0x54, 0x53, 0x2D, 0x47, 0x41, 0x50, 0x2D, 0x30, 0x36, 0x42, 0x38,
		0,0};
static uint8_t pawr_conn = 0;
#define SUBEVENT_INTERVAL 16
#define NUM_SUBEVENTS 2
#define PTS_PA_SYNC_SKIP 5

static struct bt_data ea_ad_data = {BT_DATA_FLAGS, 1, &pts_pa_data[0]};

static struct bt_data pa_ad_data[4] = {
	{BT_DATA_FLAGS, 1, &pts_pa_data[0]},
	{0x03, 4, &pts_pa_data[1]},
	{0x09, 12, &pts_pa_data[5]},
	{0x19, 2, &pts_pa_data[17]},
};
...
extern int bt_le_adv_set_enable_ext(struct bt_le_ext_adv *adv,
			 bool enable,
...
{
	SYS_LOG_INF("Advertiser[%d] %p sent %d\n",
			  ...
			  struct bt_le_ext_adv_connected_info *info)
{
	...
	SYS_LOG_INF("Advertiser[%d] %p connected by %s\n",
			...
			struct bt_le_ext_adv_scanned_info *info)
{
	...
	SYS_LOG_INF("Advertiser[%d] %p scanned by %s\n",
		    bt_le_ext_adv_get_index(adv), adv, str);
}

#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)

static int pts_pawr_set_subevent_data(int argc, char *argv[])
{
	int err = 0;
	struct bt_le_per_adv_subevent_data_params *params = &s_ctx.subevts_params[0];
	uint8_t num_subevts = 1;

	if (argc == 2)
	{
		num_subevts = (strtoul(argv[1], NULL, 0) > 1)? 2:1;
		SYS_LOG_INF("num_subevts:%d\n",num_subevts);
	}

	err = bt_le_per_adv_set_subevent_data(s_ctx.pts_adv, num_subevts, params);
	if (err != 0) {
		SYS_LOG_ERR("err: %d\n", err);
		return err;
	}
...
static void pts_pawr_data_request
			(
				struct bt_le_ext_adv *adv,
				const struct bt_le_per_adv_data_request *request
			)
{
	SYS_LOG_INF("[pawr data req]start:%d, cnt:%d\n", request->start, request->count);
	pts_pawr_set_subevent_data(0,NULL);
}

static void data_output(uint8_t *p_data, uint8_t len)
{
	uint8_t idx = 0;
	SYS_LOG_INF("data output:\n");
	for(idx = 0; idx < len; idx++)
	{
		SYS_LOG_INF("0x%02x ", p_data[idx]);
	}
	SYS_LOG_INF("\n");
}
			...
			)
{
	...
	err = bt_conn_le_create_synced(adv, &synced_param, &conn_param, &s_ctx.conn);
	if (err) {
		SYS_LOG_ERR("err:%d\n", err);
	}
}
...
static int pts_stop_pa(int argc, char *argv[])
{
	int err = 0;
	pts_test_clear_flag(PTS_TEST_FLAG_PADV);
	if (s_ctx.pts_adv) {
		err = bt_le_per_adv_stop(s_ctx.pts_adv);
		if (err != 0) {
			SYS_LOG_ERR("pa stop: %d\n", err);
		}

		err = bt_le_ext_adv_stop(s_ctx.pts_adv);
		if (err != 0) {
			SYS_LOG_ERR("ea stop: %d\n", err);
		}
	}
...
static int pts_start_pa(int argc, char *argv[])
{
	struct bt_le_per_adv_param *per_param = &s_ctx.per_adv_param;
	struct bt_le_adv_param *adv_param = &s_ctx.ext_adv_param;
	const struct bt_data *ad;
	int err = 0;

	if (argc == 2)
	{
		pawr_conn = (strtoul(argv[1], NULL, 0) > 0)? 1 : 0;
		SYS_LOG_INF("pawr_conn:%d\n",pawr_conn);
	}
	...
	adv_param->interval_min = 0x200;
	adv_param->interval_max = 0x300;
	adv_param->options = BT_LE_ADV_OPT_EXT_ADV;
	adv_param->sid = 0;
	...
	/* set subevent data */
	for (uint8_t idx = 0; idx < NUM_SUBEVENTS; idx++)
	{
		s_ctx.subevts_params[idx].subevent = idx;
		s_ctx.subevts_params[idx].response_slot_start = idx;
		s_ctx.subevts_params[idx].response_slot_count = 1;
		s_ctx.subevts_params[idx].data = &bufs[idx];
		net_buf_simple_init_with_data(&bufs[idx], pts_subevt_data, 5);
	}
#endif

	/* set adv callback */
	s_ctx.pts_adv_cb.connected = adv_connected;

#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	s_ctx.pts_adv_cb.pawr_data_request = pts_pawr_data_request;
	s_ctx.pts_adv_cb.pawr_response = pts_pawr_response;
#endif

	s_ctx.pts_adv_cb.scanned = adv_scanned;
	s_ctx.pts_adv_cb.sent = adv_sent;

	/* Create a non-connectable non-scannable advertising set */
	err = bt_le_ext_adv_create(adv_param, &s_ctx.pts_adv_cb, &s_ctx.pts_adv);
	if (err != 0) {
		SYS_LOG_ERR("adv create: %d\n", err);
		goto err_handle;
	}
#if 1
	ad = &ea_ad_data;
	err = bt_le_ext_adv_set_data(s_ctx.pts_adv, ad, 1, NULL, 0);
	if (err != 0) {
		SYS_LOG_ERR("adv set data: %d\n", err);
		goto err_handle;
	}
#endif

	/* 180ms by default */
	per_param->interval_min = 80;
	per_param->interval_max = 120;
	per_param->options = BT_LE_PER_ADV_OPT_NONE;

#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	per_param->num_subevents = NUM_SUBEVENTS;
	if (per_param->num_subevents) {
		per_param->subevent_interval = SUBEVENT_INTERVAL;
		per_param->response_slot_delay = 1;
		per_param->response_slot_spacing = 10;
		per_param->num_response_slots = NUM_SUBEVENTS;
	}
#endif

	/* Set periodic advertising procedure */
	err = bt_le_per_adv_set_param(s_ctx.pts_adv, per_param);
	if (err != 0) {
		SYS_LOG_ERR("per adv param: %d\n", err);
		goto err_handle;
	}

	ad = pa_ad_data;
	err = bt_le_per_adv_set_data(s_ctx.pts_adv, ad, sizeof(pa_ad_data)/sizeof(struct bt_data));
	if (err != 0) {
		SYS_LOG_ERR("per adv data: %d\n", err);
		goto err_handle;
	}

start_adv:

	pts_test_set_flag(PTS_TEST_FLAG_PADV);

	err = bt_le_per_adv_start(s_ctx.pts_adv);
	if (err != 0) {
		SYS_LOG_ERR("per adv enable: %d\n", err);
		goto err_handle;
	}

	struct bt_le_ext_adv_start_param *adv_start_param = &s_ctx.adv_start_param;
	adv_start_param->timeout = 0;
	adv_start_param->num_events = 0;
	/* Start extended advertising */
	err = bt_le_adv_set_enable_ext(s_ctx.pts_adv, true, adv_start_param);
	if (err != 0) {
		SYS_LOG_ERR("ext adv enable: %d\n", err);
		goto err_handle;
	}
...
static int pts_stop_ea(int argc, char *argv[])
{
	int err = 0;
...
static int pts_pa_set_info_transfer(int argc, char *argv[])
{
#if defined(CONFIG_BT_PER_ADV_SYNC_TRANSFER_SENDER)//defined(CONFIG_BT_CONN)

	int err = 0;
	if ((!s_ctx.pts_adv)||(!s_ctx.conn)) {
		SYS_LOG_ERR("not ready adv:%p,conn:%p\n", s_ctx.pts_adv,s_ctx.conn);
		return 0;
	}
	err = bt_le_per_adv_set_info_transfer(s_ctx.pts_adv, s_ctx.conn, 0);
	if (err) {
		SYS_LOG_ERR("err: %d\n", err);
	}
#endif
...
static int pts_pa_sync_transfer(int argc, char *argv[])
{
#if defined(CONFIG_BT_PER_ADV_SYNC_TRANSFER_SENDER)//defined(CONFIG_BT_CONN)

	int err = 0;
	if ((!s_ctx.pa_sync)||(!s_ctx.conn)) {
		SYS_LOG_ERR("not ready adv:%p,conn:%p\n", s_ctx.pa_sync,s_ctx.conn);
		return 0;
	}

	err = bt_le_per_adv_sync_transfer(s_ctx.pa_sync, s_ctx.conn, 0);
	SYS_LOG_ERR("ret: %d\n", err);
#endif
	return 0;
}
...
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
static void resp_buf_init(void)
{
	...
	if (err) {
		SYS_LOG_ERR("Failed to get OOB data (err %d)\n", err);
		return;
	}

	bt_addr_le_to_str(&oob.addr, addr_str, sizeof(addr_str));
	SYS_LOG_INF("Responding with own addr: %s\n", addr_str);
	...
	s_ctx.resp_buf = &rsp_buf;
}
#endif

static void pts_pa_synced(struct bt_le_per_adv_sync *sync,
				struct bt_le_per_adv_sync_synced_info *info)
{
...
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	SYS_LOG_INF("num_subevts:%d,interval:%d,resp_slot delay:%d,spacing:%d\n",
		info->num_subevents,info->subevent_interval,
		info->response_slot_delay,info->response_slot_spacing);
	...
	if (info->conn == s_ctx.conn) {
		SYS_LOG_INF("pa sync past success\n");
		if (!s_ctx.pa_sync) {
			s_ctx.pa_sync = sync;
		} else {
			SYS_LOG_INF("sync hdl diff %p(%p)\n", sync, s_ctx.pa_sync);
		}
	} else if (info->conn) {
		SYS_LOG_ERR("pa sync past who,%p(%p)\n", info->conn, s_ctx.conn);
	} else {
		/* synced pa by local */
		err = bt_le_per_adv_sync_recv_enable(sync);
		if (err) {
			SYS_LOG_ERR("en err:%d\n", err);
		}
	}
}

static void pts_pa_recv(struct bt_le_per_adv_sync *sync,
				...
				struct net_buf_simple *buf)
{
...
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	SYS_LOG_INF("evt counter:%d,sub evt:%d\n", info->periodic_event_counter, info->subevent);
	...
	err = bt_le_per_adv_set_response_data(sync, &rsp_params, s_ctx.resp_buf);
	if (err) {
		SYS_LOG_ERR("Failed to send response (err %d)\n", err);
	}
#endif
}

static void pts_pa_term(struct bt_le_per_adv_sync *sync,
				const struct bt_le_per_adv_sync_term_info *info)
{
	int err = 0;
	SYS_LOG_INF("sid:%d,reason:%d\n", info->sid, info->reason);
	err = hostif_bt_le_scan_stop();
	if (err != 0) {
		SYS_LOG_ERR("scan_stop: %d\n", err);
	}
	s_ctx.pa_sync = NULL;
	pts_test_clear_flag(PTS_TEST_FLAG_PADV);
...
static void pts_scan_recv(const struct bt_le_scan_recv_info *info,
				struct net_buf_simple *ad)
{
	...
	if (s_ctx.pa_sync) {
		SYS_LOG_INF("already sync pa_sync:%p\n",s_ctx.pa_sync);
		return;
	}

	if (!memcmp(s_ctx.peer_addr.a.val, info->addr->a.val, PTS_MAC_ADDR_SIZE))
	{
		le_addr_output(&s_ctx.peer_addr);
		SYS_LOG_INF("type: %d, props: 0x%x, int: %d, rssi: %d, phy: %d, %d\n",
			info->adv_type, info->adv_props, info->interval,
			info->rssi, info->primary_phy, info->secondary_phy);

		bt_addr_le_copy(&sync_param.addr, info->addr);
		sync_param.options = BT_LE_PER_ADV_SYNC_OPT_FILTER_DUPLICATE;
		sync_param.sid = info->sid;
		sync_param.skip = 5;
		sync_param.timeout = 400;//interval_to_sync_timeout(info->interval);
		err = bt_le_per_adv_sync_create(&sync_param, &s_ctx.pa_sync);
		if (err != 0) {
			SYS_LOG_ERR("err: %d\n", err);
			return;
		}
...
static void pts_scan_timeout(void)
{
	SYS_LOG_INF("scan timeout\n");
}

static int pts_start_scan(int argc, char *argv[])
{
	struct bt_le_scan_param *param = &s_ctx.scan_param;
	int err;
	...
	bt_addr_le_t addr_zero = {0};
	if (!memcmp((void*)&s_ctx.peer_addr, (void *)&addr_zero, sizeof(s_ctx.peer_addr))) {
		SYS_LOG_ERR("addr err!\n");
		return 0;
	}
	le_addr_output(&s_ctx.peer_addr);
	...
	err = hostif_bt_le_scan_start(param, NULL);
	if (err) {
		SYS_LOG_ERR("scan_start err: %d\n", err);
		pts_test_clear_flag(PTS_TEST_FLAG_PADV);
	}
...
static int pts_stop_scan(int argc, char *argv[])
{
	int err = 0;
	pts_test_clear_flag(PTS_TEST_FLAG_PADV);
	err = hostif_bt_le_scan_stop();
...
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
static int pts_create_synced(int argc, char *argv[])
{
	struct bt_conn_le_create_synced_param synced_param;
	struct bt_le_conn_param conn_param;
	...
	synced_param.peer = &s_ctx.peer_addr;
	...
	int err = bt_conn_le_create_synced(s_ctx.pts_adv, &synced_param, &conn_param, &s_ctx.conn);
	if (err) {
		SYS_LOG_ERR("err:%d\n", err);
	}
	return 0;
...
static int pts_pa_sync_past(int argc, char *argv[])
{
	int err;
	uint8_t report_flag = 1;
	struct bt_le_per_adv_sync_transfer_param param = { 0 };
	if (!s_ctx.conn) {
		SYS_LOG_ERR("not ready conn:%p\n", s_ctx.conn);
		return 0;
	}
	...
	{
		report_flag = 0;
		SYS_LOG_INF("just sync, do not report\n");
	} else {
		SYS_LOG_INF("sync and report\n");
	}

	pts_test_set_flag(PTS_TEST_FLAG_PADV);

	param.skip = PTS_PA_SYNC_SKIP;
	param.timeout = 100;//1s

	if (report_flag) {
		err = bt_le_per_adv_sync_transfer_subscribe(s_ctx.conn, &param);
	} else {
		err = bt_le_per_adv_sync_transfer_unsubscribe(s_ctx.conn);
	}
...
{
	struct bt_le_scan_cb *scan_cb = NULL;
	struct bt_le_per_adv_sync_cb *pa_sync_cb = NULL;
	...
	scan_cb = &s_ctx.scan_cb;
	scan_cb->recv = pts_scan_recv;
	scan_cb->timeout = pts_scan_timeout;

	/* register before scan start */
	bt_le_scan_cb_register(scan_cb);

	pa_sync_cb = &s_ctx.pa_sync_cb;
	pa_sync_cb->synced = pts_pa_synced;
	pa_sync_cb->recv = pts_pa_recv;
	pa_sync_cb->term = pts_pa_term;
	pa_sync_cb->biginfo = NULL;
...
int bt_manager_pts_test_stop(void)
{
	...
	bt_le_scan_cb_unregister(&s_ctx.scan_cb);
	bt_le_per_adv_sync_cb_unregister(&s_ctx.pa_sync_cb);
...
{
	/* Core stack cases */
	{ "get_attr_handle_by_uuid", pts_get_attr_handle_by_uuid, "get attr handle by uuid(0xXXXX UUID)"},

#ifdef CONFIG_BT_PAWR_PAST
	{ "start_pa", pts_start_pa, "start pa(1-conn,0-no_conn)"},
	{ "stop_pa", pts_stop_pa, "stop PA"},
	{ "stop_ea", pts_stop_ea, "stop EA"},
	{ "start_scan", pts_start_scan, "start scan"},
	{ "stop_scan", pts_stop_scan, "stop scan"},
#if defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	{ "pawr_set_subevent_data", pts_pawr_set_subevent_data, "pawr set subevent data"},
	{ "create_synced", pts_create_synced, "create_synced(create connection)"},
#endif /*defined(CONFIG_BT_PER_ADV_RSP) && defined(CONFIG_BT_PER_ADV_SYNC_RSP)*/
	{ "pa_set_info_transfer", pts_pa_set_info_transfer, "set pa info transfer"},
	{ "pa_sync_transfer", pts_pa_sync_transfer, "pa sync transfer"},
	{ "pa_sync_past", pts_pa_sync_past, "pa sync past"},
#endif
	...
	/* BT connect/disconnect cmd*/
	{ "bt_create_acl", bt_manager_create_conn, "create bt acl "},
	{ "bt_dis_acl", bt_manager_disconn_conn, "disconn bt acl "},
