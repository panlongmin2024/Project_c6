...
struct bt_pbap_client_user_cb {
	...
	void (*recv)(struct bt_conn *conn, uint8_t user_id, struct parse_vcard_result *result, uint8_t array_size);
};

uint8_t bt_pbap_client_get_phonebook(struct bt_conn *conn, char *path, struct bt_pbap_client_user_cb *cb);
int bt_pbap_client_abort_get(struct bt_conn *conn, uint8_t user_id);

/* Function interface for pts test, only for pts test, not for normal used */
int bt_pts_pbap_client_get_phonebook_size(struct bt_conn *conn, uint8_t user_id);
int bt_pts_pbap_client_get_vcard(struct bt_conn *conn, uint8_t user_id);
int bt_pts_pbap_client_get_final(struct bt_conn *conn, uint8_t user_id);
int bt_pts_pbap_client_abort(struct bt_conn *conn, uint8_t user_id);
int bt_pts_pbap_client_disconnect(struct bt_conn *conn, uint8_t user_id);
