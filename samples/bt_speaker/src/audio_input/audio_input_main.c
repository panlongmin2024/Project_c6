...
static void _audio_input_restore_play_state(u8_t init_play_state)
{
	...
	if (init_play_state == AUDIO_INPUT_STATUS_PLAYING) {
		...
		thread_timer_init(&p_audio_input->monitor_timer,
				  audio_input_delay_resume, NULL);
		thread_timer_start(&p_audio_input->monitor_timer, 2500, 0); //wait tts finish, then start app player.
...
void audio_input_energy_detect(struct thread_timer *ttimer, void *expiry_fn_arg)
{
	...
	if (audio_input->player && audio_input->playing) {
		...
		media_player_get_energy_detect(audio_input->player,
					       &energy_detect_info);
