...
static const uint8_t ectt_tag_table[9] = {
	...
	MEDIA_DATA_TAG_DECODE_IN, MEDIA_DATA_TAG_DECODE_OUT1,
...
static struct ETCC_data *_etcc_data_alloc(void)
{
	...
	if (media_mem_get_cache_pool_size(TOOL_ECTT_BUF, AUDIO_STREAM_MUSIC) <
	    ECTT_DUMP_BACKSTORE_BUF_SIZE) {
	...
	data->dump_backstore =
	    media_mem_get_cache_pool(TOOL_ECTT_BUF, AUDIO_STREAM_MUSIC);

	/* must not less than one transfer size */
	if (!_realloc_ectt_stub_parambuf
	    (data, sizeof(struct ECTT_transfer) + ECTT_TRANSFER_SIZE)) {
		app_mem_free(data);
		return NULL;
	}
...
static void _etcc_data_free(struct ETCC_data *data)
{
	for (int i = 0; i < ARRAY_SIZE(data->dump_bufs); i++) {
		if (data->dump_bufs[i]) {
			media_player_dump_data(NULL, 1, &data->dump_tags[i],
					       NULL);
...
static int _ectt_stub_read(struct ETCC_data *data, uint16_t opcode, void *buf,
			   size_t size)
{
...
static int _ectt_stub_write(struct ETCC_data *data, uint16_t opcode,
			    const void *buf, size_t size, int msel)
{
	size_t ext_size = (msel >= 0) ? sizeof(struct ECTT_transfer) : 0;
	stub_ext_param_t param = {.opcode = opcode,.payload_len =
		    size + ext_size, };
	int ret = 0;
	...
	if (size > 0) {
		if (msel >= 0) {
			struct ECTT_transfer *transfer =
			    (struct ECTT_transfer *)&param.
			    rw_buffer[sizeof(stub_ext_cmd_t)];
			transfer->mod = (uint16_t) msel;
			transfer->size = size;
			ret =
			    acts_ringbuf_get((void *)buf, transfer->data, size);
			if (ret != size) {
				SYS_LOG_ERR("no enough data");
				return ret;
			}
		} else {
			memcpy(&param.rw_buffer[sizeof(stub_ext_cmd_t)], buf,
			       size);
		}
	}
	...
	if (sys_get_be16(&param.rw_buffer[1]) != STUB_CMD_ECTT_ACK) {
		SYS_LOG_ERR("invalid ack 0x%02x",
			    sys_get_be16(&param.rw_buffer[1]));
		return -EINVAL;
	}
...
{
	if (_ectt_stub_read
	    (data, STUB_CMD_ECTT_GET_STATUS, status, sizeof(*status)))
		return -EIO;
...
static int _get_modsel(struct ETCC_data *data, struct ECTT_modsel *modsel)
{
	...
	if (_ectt_stub_read
	    (data, STUB_CMD_ECTT_GET_MODSEL_FLAG, &flag, sizeof(flag)))
		return -EIO;
	...
	if (_ectt_stub_read
	    (data, STUB_CMD_ECTT_GET_MODSEL_VALUE, modsel, sizeof(*modsel)))
		return -EIO;
...
static int _start_upload_data(struct ETCC_data *data)
{
	...
	for (int i = 0; i < min(ECTT_MODSEL_COUNT, ARRAY_SIZE(ectt_tag_table));
	     i++) {
		if (!data->modsel.bSelArray[i])
			continue;

		if (num >= ARRAY_SIZE(data->dump_bufs)) {
			SYS_LOG_WRN("exceed max %d tags",
				    (unsigned int)ARRAY_SIZE(data->dump_bufs));
			break;
		}

		debug_buff =
		    data->dump_backstore + num * ECTT_TRANSFER_SIZE * 2;
		data->dump_bufs[num] =
		    acts_ringbuf_init_ext(debug_buff, ECTT_TRANSFER_SIZE * 2);
		if (!data->dump_bufs[num]) {
			SYS_LOG_ERR("acts_ringbuf_init_ext failed");
			break;
		}

		SYS_LOG_INF("start idx=%d, tag=%u, buff=%p, msel=%d", i,
			    ectt_tag_table[i], data->dump_bufs[num], i);
...
static int _upload_data(struct ETCC_data *data)
{
	...
	do {
		...
		for (int i = 0; i < ARRAY_SIZE(data->dump_bufs); i++) {
			if (!data->dump_bufs[i]
			    || acts_ringbuf_length(data->dump_bufs[i]) <
			    ECTT_TRANSFER_SIZE)
				continue;

			ret =
			    _ectt_stub_write(data, STUB_CMD_ECTT_UPLOAD_DATA,
					     data->dump_bufs[i],
					     ECTT_TRANSFER_SIZE,
					     data->dump_msel[i]);
			if (ret <= 0)
...
static int _stop_upload_data(struct ETCC_data *data)
{
	for (int i = 0; i < ARRAY_SIZE(data->dump_bufs); i++) {
		if (data->dump_bufs[i]) {
			SYS_LOG_INF("stop idx=%d, tag=%u, buff=%p", i,
				    data->dump_tags[i], data->dump_bufs[i]);
			media_player_dump_data(NULL, 1, &data->dump_tags[i],
					       NULL);
			acts_ringbuf_destroy_ext(data->dump_bufs[i]);
			data->dump_bufs[i] = NULL;
...
{
	return _ectt_stub_write(data, STUB_CMD_ECTT_UPLOAD_SAMPLERATE,
				&sample_rate, sizeof(sample_rate), -1);
}
