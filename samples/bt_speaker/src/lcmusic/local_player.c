...
int mplayer_update_media_info(struct local_player_t *lcplayer, media_info_t *info)
{
	...
	SYS_LOG_INF("total_time=%d, sample_rate=%d, channels=%d, avg_bitrate=%d\n",
...
#endif
        audio_policy_set_bis_link_delay_ms(broadcast_get_bis_link_delay(lcmusic->qos));
    }
#endif
...
#if CONFIG_BMS_LCMUSIC_APP
    SYS_LOG_INF("bms %d bms_source:%d\n", lcmusic_is_bms_mode(), lcmusic->bms_source);

    if (lcmusic_is_bms_mode() && 1 == lcmusic->bms_source) {
        bt_manager_broadcast_stream_tws_sync_cb_register_1(lcmusic->chan,
...
#if CONFIG_BMS_LCMUSIC_APP
    if (lcmusic_is_bms_mode()) {
        bt_manager_broadcast_stream_tws_sync_cb_register_1(lcmusic->chan, NULL, NULL);
    }
#endif
