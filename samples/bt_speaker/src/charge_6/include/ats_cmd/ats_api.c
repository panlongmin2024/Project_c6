...
int ats_usb_cdc_acm_cmd_response(struct device *dev, uint8_t *buf, int size)
{
    ...
    ats_usb_cdc_acm_write_data(dev, ats_get_cmd_resp_buf(), size);
...
{
    k_sleep(100);
    SYS_LOG_INF("----> ats_sys_reboot %d\n",__LINE__);
    sys_reboot_by_user(REBOOT_REASON_USER_ENTER_CHARGE);
    return 0;
}
...
int ats_usb_cdc_acm_key_check_report(struct device *dev, uint32_t key_value)
{
    ...
    if (p_ats_ctx->ats_enable_enter_key_check)
    {
        ...
        ats_usb_cdc_acm_cmd_response_at_data(
            dev, ATS_CMD_RESP_ENTER_KEY_CHECK, sizeof(ATS_CMD_RESP_ENTER_KEY_CHECK)-1, 
