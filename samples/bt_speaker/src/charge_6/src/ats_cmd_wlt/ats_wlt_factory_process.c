...
/* number of GPIO that need to test */
static const u8_t ats_wlt_gpio_array[] = {
0,	1,	
//2,	3,//tx rx
...
#if CONFIG_WLT_ATS_NEED_COMM
			/* not need malloc thread_stack, not need malloc msg_buf */
			p_ats_wlt_info = malloc(sizeof(struct _wlt_driver_ctx_t));
			if(p_ats_wlt_info == NULL){
				SYS_LOG_ERR("it's not enough ram\n");
				return -1;				
			}else{
				SYS_LOG_INF("it has enough ram,then init it\n");
...
static int ats_wlt_command_shell_handler(struct device *dev, u8_t *buf, int size)
{
	...
	memset(p_ats_wlt_info->data_buf, 0, ATS_WLT_UART_RX_LEN_MAX);
	return 0;
